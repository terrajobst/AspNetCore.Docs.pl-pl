---
uid: aspnet/overview/owin-and-katana/index
title: OWIN i Katana | Dokumentacja firmy Microsoft
author: rick-anderson
description: Katana jest elastyczny zestaw składników do tworzenia i obsługiwania interfejsu Open Web dla aplikacji sieci web opartych na .NET OWIN.
ms.author: aspnetcontent
manager: wpickett
ms.date: 05/30/2013
ms.topic: article
ms.assetid: 37e005e9-2f1c-4039-b6ef-15612e0fdbc9
ms.technology: ''
ms.prod: .net-framework
msc.legacyurl: /aspnet/overview/owin-and-katana
msc.type: chapter
ms.openlocfilehash: a97cf97328ea6e4f7fe527ea7c9068ec60341d0b
ms.sourcegitcommit: 9a9483aceb34591c97451997036a9120c3fe2baf
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 11/10/2017
ms.locfileid: "26563636"
---
<a name="owin-and-katana"></a>OWIN i Katana
====================
> Katana jest elastyczny zestaw składników do tworzenia i obsługiwania interfejsu Open Web dla aplikacji sieci web opartych na .NET OWIN.


- [Przegląd projektu Katana](an-overview-of-project-katana.md)
- [Wprowadzenie do korzystania z OWIN i Katana](getting-started-with-owin-and-katana.md)
- [Przykłady Katana](katana-samples.md)
- [Wykrywanie klasy uruchamiania OWIN](owin-startup-class-detection.md)
- [Włączanie uwierzytelniania systemu Windows w kolekcji Katana](enabling-windows-authentication-in-katana.md)
- [Oprogramowanie pośredniczące OWIN w zintegrowanym potoku usług IIS](owin-middleware-in-the-iis-integrated-pipeline.md)
- [Serwera autoryzacji OAuth 2.0 OWIN](owin-oauth-20-authorization-server.md)
- [Host OWIN w roli procesu roboczego platformy Azure](host-owin-in-an-azure-worker-role.md)
