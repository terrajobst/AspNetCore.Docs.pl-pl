...
ms.locfileid: "26565559"
---
<a name="dependency-injection-in-signalr"></a>Iniekcji zależności w bibliotece SignalR
====================
...
`StockTickerHub`zawiera odwołanie do `StockTicker` jako pojedynczej, podczas gdy `StockTicker` zawiera odwołanie do **IHubConnectionContext** dla `StockTickerHub`. Ten interfejs używa do komunikacji z `StockTickerHub` wystąpień. (Aby uzyskać więcej informacji, zobacz [serwera emisji z ASP.NET SignalR](../getting-started/tutorial-server-broadcast-with-signalr.md).)
...
Usuń konstruktora bez parametrów z `StockTickerHub`. Zamiast tego należy zawsze używamy Podpisane utworzyć koncentratora.
...
Otwórz klasy Startup.cs aplikacji (albo utworzony ręcznie zgodnie z instrukcjami pakietu `readme.txt`, i który został utworzony przez dodawanie uwierzytelniania do projektu). W `Startup.Configuration` metody tworzenia kontenera Ninject, który wywołuje Ninject *jądra*.
...
Przekaż do mechanizmu rozpoznawania zależności **MapSignalR** metody przez dodanie konfiguracji Centrum:

[!code-csharp[Main](dependency-injection/samples/sample19.cs)]

Zaktualizuj metodę Startup.ConfigureSignalR w klasie uruchamiania próbki o nowy parametr:

[!code-csharp[Main](dependency-injection/samples/sample20.cs)]

Teraz SignalR będzie używać programu rozpoznawania nazw, określonych w **MapSignalR**, zamiast domyślny program rozpoznawania nazw.

W tym miejscu jest kompletny kod dla `Startup.Configuration`.
...
Aplikacja ma dokładnie tak samo jak przed. (Aby uzyskać opis, zobacz [serwera emisji z ASP.NET SignalR](../getting-started/tutorial-server-broadcast-with-signalr.md).) Firma Microsoft nie zostały zmienione zachowanie; po prostu łatwiejsza kod do testowania, obsługa i rozwijać.
