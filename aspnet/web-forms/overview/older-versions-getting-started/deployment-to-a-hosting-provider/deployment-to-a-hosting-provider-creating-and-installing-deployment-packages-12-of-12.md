...
Aby włączyć tę aplikację wyświetlić szczegółowe komunikaty o błędach, gdy jest uruchamiany na zdalnym hoście, edytowania pliku Web.config, aby ustawić `customErrors` tryb wyłączone, należy ponownie wdrożyć aplikację i ponownie uruchomić aplikację:

1. Jeśli plik Web.config ma `customErrors` element `system.web` elementu, zmień `mode` atrybutu na "off". W przeciwnym razie Dodaj `customErrors` element `system.web` element z `mode` atrybutu na "wyłączone", jak pokazano w poniższym przykładzie:
...
4. Po rozwiązaniu błąd, Przywróć oryginalną `customErrors` ustawienie i wdrożenie aplikacji.
...
Również może być konieczne ręczne ustawienie programu .NET Framework w wersji domyślnej puli aplikacji. Aby uzyskać więcej informacji, zobacz [wdrażania usług IIS jako środowisko testowe](deployment-to-a-hosting-provider-deploying-to-iis-as-a-test-environment-5-of-12.md) samouczka.
...
Otwórz *Web.config* plik w witrynie wdrożone i sprawdź, czy wartości parametrów połączeń rozpoczynać się od `$(ReplacableToken_`, jak w poniższym przykładzie:

[!code-xml[Main](deployment-to-a-hosting-provider-creating-and-installing-deployment-packages-12-of-12/samples/sample9.xml)]

Jeśli parametry połączenia wyglądać w tym przykładzie, Edytuj plik projektu i dodaj następującą właściwość, aby `PropertyGroup` element, który jest wszystkie konfiguracje kompilacji:
...
Istnieje wiele przyczyn błędów 500, ale jest jedną z możliwych przyczyn Jeśli wykonujesz te samouczki, możesz zaznaczyć XML element w niewłaściwym miejscu w jednym z plików transformacji XML. Na przykład, czy ten błąd wystąpi Jeśli umieścisz przekształcania, która wstawia `<location>` pod `<system.web>` zamiast bezpośrednio pod `<configuration>`. Rozwiązanie jest w takim przypadku można poprawić plik transformacji XML i wdrożenie.
...
Konto użytkownika, którego używasz do wykonywania wdrożenia nie ma uprawnień do tworzenia użytkowników lub ról. Na przykład firma może przypisywać `db_datareader`, `db_datawriter`, i `db_ddladmin` ról z kontem użytkownika, który konfiguruje dla Ciebie. Te są wystarczające do tworzenia większości obiektów bazy danych, ale nie do tworzenia użytkowników lub ról. Jest jednym ze sposobów uniknąć tego błędu, wyłączając użytkowników i ról z bazy danych. Można to zrobić, edytując `PreSource` elementu dla bazy danych użytkownika są generowane automatycznie skryptu co zawierają one następujące atrybuty:
...
Jeśli podczas instalowania pakietu przy użyciu *pliku deploy.cmd* pliku z `t` opcji (test), zostanie wyświetlony następujący komunikat o błędzie:

[!code-console[Main](deployment-to-a-hosting-provider-creating-and-installing-deployment-packages-12-of-12/samples/sample18.cmd)]

### <a name="possible-cause-and-solution"></a>Możliwa przyczyna i rozwiązanie

Komunikat o błędzie oznacza, że polecenie nie może utworzyć raport z testów. Jednak polecenie może działać, jeśli używasz `y` opcji (rzeczywista instalacja). Komunikat wskazuje tylko, że występuje problem z uruchamianiem polecenia w trybie testowym.
...
Podczas próby wdrożenia pojawić się następujący komunikat o błędzie:
 ...
 Błąd: Strumień danych "sitemanifest/dbFullSql [@path="C:\TEMP\AdventureWorksGrant.sql']/sqlScript"nie jest jeszcze dostępna. Puli aplikacji, które próbujesz użyć ma jako wartość właściwości "managedRuntimeVersion" wartość "v2.0". Ta aplikacja wymaga "v4.0". 
...
Pomyślnie publikowania projektu sieci web, przeznaczonego dla platformy ASP.NET 4.5, ale po uruchomieniu aplikacji (z `customErrors` tryb ustawiony na "off" w pliku Web.config) następujący błąd:
