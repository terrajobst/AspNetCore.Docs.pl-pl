...
`DisplayWebConfig` Używa metody [ `File` klasy](https://msdn.microsoft.com/en-us/library/system.io.file.aspx) otworzyć s aplikacji `Web.config` pliku [ `StreamReader` klasy](https://msdn.microsoft.com/en-us/library/system.io.streamreader.aspx) Aby przeczytać jego zawartość do ciągu i [ `Path` klasy](https://msdn.microsoft.com/en-us/library/system.io.path.aspx) do ścieżki fizycznej do generowania `Web.config` pliku. Te trzy klasy zostały znalezione w [ `System.IO` przestrzeni nazw](https://msdn.microsoft.com/en-us/library/system.io.aspx). W związku z tym, należy dodać `using` `System.IO` instrukcji u góry klasy związane z kodem, lub te klasy nazw z prefiksem `System.IO.` .
