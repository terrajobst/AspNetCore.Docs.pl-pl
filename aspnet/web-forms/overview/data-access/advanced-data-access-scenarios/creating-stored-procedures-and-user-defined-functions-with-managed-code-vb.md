...
Należy pamiętać, że procedura składowana jest zaimplementowany jako `Shared` metodę `Partial` pliku klasy o nazwie `StoredProcedures`. Ponadto `GetDiscontinuedProducts` zostanie nadany metody [ `SqlProcedure` atrybutu](https://msdn.microsoft.com/en-us/library/microsoft.sqlserver.server.sqlprocedureattribute.aspx), która oznacza metodę jako procedury składowanej.
