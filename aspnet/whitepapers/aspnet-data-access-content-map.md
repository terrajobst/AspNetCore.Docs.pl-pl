...
- [Opcje dostępu do danych w programie ASP.NET](https://msdn.microsoft.com/en-us/library/ms178359.aspx) (MSDN). Wprowadzenie do opcji dostępu do danych relacyjnych baz danych programu ASP.NET oraz wskazówki na temat wybierz platformy i metody, które są odpowiednie dla danego scenariusza dostępu.
...
- [Entity Framework programowanie podejścia](https://msdn.microsoft.com/en-us/library/ms178359.aspx#dbfmfcf) (MSDN). Wskazówki na temat wybierania programu Entity Framework podejście do tworzenia pierwszej bazy danych Model First "lub" Code First.
...
- [Entity Framework - kod najpierw do istniejącej bazy danych](https://msdn.microsoft.com/en-us/data/jj200620). MSDN. Wideo i wskazówki, która przedstawia sposób użycia Code First z istniejącej bazy danych.
- [Centrum deweloperów danych - programu Entity Framework](https://msdn.microsoft.com/data/ef). MSDN. Przewodnik dotyczący dokumentacji programu Entity Framework, który został utworzony i jest obsługiwany przez zespół Entity Framework, zobacz [wprowadzenie](https://msdn.microsoft.com/en-us/data/ee712907) łącza.
...
- [Omówienie wdrażania w sieci Web dla platformy ASP.NET i Visual Studio](https://msdn.microsoft.com/en-us/library/dd394698.aspx#dbdeployment). Zobacz **Konfigurowanie wdrażania bazy danych w programie Visual Studio** sekcji, aby uzyskać informacje o sposobie migracje Code First jest zintegrowany funkcje wdrażania sieci web programu Visual Studio.
- [Centrum deweloperów danych - migracje Code First](https://msdn.microsoft.com/en-us/data/jj591621) (MSDN). Dokumentacja migracje zespołu programu Entity Framework.
...
- [Centrum deweloperów danych - ładowanie powiązanych jednostek](https://msdn.microsoft.com/en-us/data/jj574232) (MSDN). Dane dotyczące dokumentacji ładowania zespołu programu Entity Framework.
...
- [Zagadnienia dotyczące wydajności Entity Framework 5](https://msdn.microsoft.com/en-us/data/hh949853) (MSDN).
- [Zagadnienia dotyczące wydajności (Entity Framework)](https://msdn.microsoft.com/en-us/library/cc853327) (MSDN).
...
- [Dane w Centrum deweloperów — wzorce optymistycznej współbieżności](https://msdn.microsoft.com/en-us/data/jj592904) (MSDN). Dokumentacja współbieżności zespołu programu Entity Framework.
...
- [Kontrolki serwera sieci Web źródła danych](https://msdn.microsoft.com/en-us/library/ms247258.aspx) (MSDN).
...
- [Formanty serwera sieci Web powiązanych z danymi](https://msdn.microsoft.com/en-us/library/ms228214.aspx) (MSDN).
- [Omówienie wyrażenia wiązania danych](https://msdn.microsoft.com/en-us/library/ms178366.aspx) (MSDN). To strony tylko obejmuje **Eval** i **powiązać**; nie został jeszcze zaktualizowany do uwzględnienia **elementu** i **metody BindItem**.

<a id="sqlserver"></a>

## <a name="working-with-sql-server-databases"></a>Praca z bazy danych programu SQL Server

- [Funkcje bazy danych programu SQL Server](https://msdn.microsoft.com/en-us/library/hh230827.aspx) (MSDN). Aby uzyskać ogólne wprowadzenie do różnych tematów programu SQL Server zobacz wpisy w tym typie w spisie treści.
- [Wersje programu SQL Server](https://msdn.microsoft.com/en-us/library/ms178359.aspx#sqlserver) (MSDN). Podsumowanie dostępne wersje programu SQL Server, z łącza do dodatkowych informacji o każdym z nich.)
- [Parametry połączenia serwera SQL dla aplikacji sieci Web ASP.NET](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN).
- [Dla aplikacji sieci Web ASP.NET przy użyciu programu SQL Server Compact](https://msdn.microsoft.com/en-us/library/ms247257.aspx) (MSDN).
...
- [Instalowanie przykładowej bazy danych](https://github.com/Microsoft/sql-server-samples/blob/master/samples/databases/adventure-works/README.md). Oprócz metod pokazane, możesz również pobrać jednego z przykładowych plików .mdf do aplikacji\_danych folderu projektu sieci web, przekonwertować bazy danych LocalDB, a następnie utworzyć parametry połączenia bazy danych LocalDB. Aby dowiedzieć się, jak to zrobić, zobacz [porady: uaktualnienie do LocalDB](https://msdn.microsoft.com/en-us/library/hh873188.aspx).
...
- [Program SQL Server Express 2012 LocalDB](https://msdn.microsoft.com/en-us/library/hh510202(v=sql.110).aspx) (MSDN). Oficjalna MSDN wprowadzenie do LocalDB.
- [Parametry połączenia serwera SQL dla aplikacji sieci Web ASP.NET](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN).
- [Porady: uaktualnienie do LocalDB](https://msdn.microsoft.com/en-us/library/hh873188.aspx) (MSDN). Jak przeprowadzić migrację plików .mdf ze starszej wersji programu SQL Server Express do LocalDB. Masz również przechodzić przez ten proces, jeśli pobranie jednej z [bazy danych programu SQL Server 2012 przykładowej](https://go.microsoft.com/fwlink/?linkid=117483).
...
- [Parametry połączenia serwera SQL dla aplikacji sieci Web ASP.NET](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN). Jeśli ustawienie parametrów połączenia AttachDBFileName użytkownik korzysta z programu SQL Server Express, zobacz zwłaszcza sekcję wystąpienia użytkownika tej strony.
...
- [Moja aplikacja sieci web platformy ASP.NET można użyć bazy danych programu SQL Server Express w środowisku produkcyjnym?](https://msdn.microsoft.com/en-us/library/jj653753.aspx#sql_express_in_production) (MSDN).
...
- [Baza danych Azure SQL z systemem Windows](https://msdn.microsoft.com/en-us/library/windowsazure/ee336279.aspx) (MSDN). Węzeł najwyższego poziomu spisu treści dla bazy danych SQL w witrynie MSDN.
...
- [Obsługa błędu przejściowego bloku aplikacji](https://msdn.microsoft.com/en-us/library/hh680934(v=PandP.50).aspx). Platforma, która umożliwia obsługę błędów sieci i błędów dotyczących połączeń wynikających z ograniczenia przepustowości. Dostępne w pakiecie NuGet: [Enterprise biblioteki 5.0 - bloku aplikacji obsługi błędów przejściowych](http://nuget.org/packages/EnterpriseLibrary.WindowsAzure.TransientFaultHandling).
- [Wprowadzenie do korzystania z bazy danych SQL i programu Entity Framework](https://msdn.microsoft.com/en-us/data/jj556244) (MSDN).
- [Zestaw szkoleniowy Azure Windows](https://www.microsoft.com/en-us/download/details.aspx?id=8396) (Centrum pobierania Microsoft). Obejmuje praktyczne labs bazy danych SQL.
- [Forum społeczności bazy danych Azure SQL dla systemu Windows](https://social.msdn.microsoft.com/Forums/en-US/ssdsgetstarted/threads).
- [Przenoszenie do systemu Windows Azure SQL Database](https://msdn.microsoft.com/en-us/library/ff803375.aspx) (MSDN). Rozdziału jeden kompleksowe scenariusza na trasie przez zespół Microsoft Patterns and Practices. Dlaczego warto migracji obejmuje i przeprowadzanie migracji z programu SQL Server do bazy danych SQL.
...
- [Migracji danych do systemu Windows Azure SQL Database: narzędzi i technik](https://msdn.microsoft.com/en-us/library/windowsazure/hh694043.aspx) (MSDN). Zawiera sekcje porównywania programu SQL Server z bazą danych SQL i zapewnianie wskazówek w przypadku migracji z programu SQL Server z bazą danych SQL.
...
- [Ograniczenia funkcji serwera SQL (baza danych Azure SQL z systemem Windows)](https://msdn.microsoft.com/en-us/library/windowsazure/ff394115.aspx) (MSDN).
- [Windows Azure Table Storage i baza danych Azure SQL z systemem Windows — porównywane i odróżniające](https://msdn.microsoft.com/en-us/library/jj553018.aspx) (MSDN). Aplikacji wdrażanej w systemie Windows Azure magazyn tabel Azure systemu Windows może być alternatywę dla bazy danych SQL Azure z systemem Windows. Ten temat ułatwia podjęcie decyzji o następujących alternatyw.
- [Baza danych Azure SQL z systemem Windows](https://msdn.microsoft.com/en-us/library/windowsazure/ee336279) (MSDN).
- [Wskazówki i ograniczenia (baza danych Azure SQL z systemem Windows)](https://msdn.microsoft.com/en-us/library/windowsazure/ff394102.aspx)

<a id="nosql"></a>

## <a name="working-with-nosql-database-management-systems"></a>Praca z systemy zarządzania bazami danych NoSQL

- [Usługi danych systemu Windows Azure](https://www.windowsazure.com/en-us/develop/net/data/) (witryny Microsoft Azure). Zobacz [Przewodnik po funkcji usługi tabel](https://docs.microsoft.com/azure/) i **danych Big Data** części strony.
...
- [Opcje dostępu do danych w programie ASP.NET](https://msdn.microsoft.com/en-us/library/ms178359.aspx#linq) (MSDN). Zawiera wprowadzenie do LINQ.
...
- [Szablony projektu z danymi dynamicznymi](https://msdn.microsoft.com/en-us/library/jj822927.aspx#dynamicdata) (MSDN). Wskazówki dotyczące użycie danych dynamicznych projektów.
...
- [Zabezpieczanie dostępu do danych w programie ASP.NET](https://msdn.microsoft.com/en-us/library/ms178375.aspx) (MSDN).
- [Zagadnienia dotyczące zabezpieczeń (Entity Framework)](https://msdn.microsoft.com/en-us/library/cc716760.aspx) (MSDN).
- [Porady: Secure parametry połączenia, korzystając z kontrolki źródła danych](https://msdn.microsoft.com/en-us/library/ms178372.aspx) (MSDN).

<a id="optimizingdataaccess"></a>

## <a name="optimizing-data-access-performance"></a>Optymalizowanie wydajności dostępu do danych

- [Omówienie wydajności programu ASP.NET](https://msdn.microsoft.com/en-us/library/cc668225.aspx) (MSDN).
- [ASP.NET buforowanie](https://msdn.microsoft.com/en-us/library/xsbfdd8c.aspx) (MSDN).
- [Poprawianie wydajności ASP.NET](https://msdn.microsoft.com/en-us/library/ff647787) (MSDN). Znajduje się ostrzeżenie "Wycofane zawartość" w górnej części strony, ale większość informacji jest nadal ważna i nie ma nie można porównywać pod względem zaktualizowanego zasobu.
- [Poprawianie wydajności serwera SQL](https://msdn.microsoft.com/en-us/library/ff647793) (MSDN). Tym samym komentarz jako poprzedniej konsolidacji.
...
- [Uzyskiwanie dostępu do danych za pośrednictwem usługi sieci Web](https://msdn.microsoft.com/en-us/library/ms178359.aspx#webservice) (MSDN). Wskazówki dotyczące użycie interfejsu API sieci Web lub usługi WCF.
- [Wprowadzenie do korzystania z interfejsu API sieci Web ASP.NET](../web-api/index.md).
- [Usługi danych WCF](https://msdn.microsoft.com/en-us/data/bb931106) (MSDN).

<a id="additional"></a>

## <a name="additional-resources"></a>Dodatkowe zasoby

- [Dostęp do danych programu ASP.NET — często zadawane pytania](https://msdn.microsoft.com/en-us/library/jj653753.aspx) (MSDN).
- [Samouczki - danych formularzy sieci Web ASP.NET](../web-forms/overview/data-access/index.md). Większość tego samouczka jest stosunkowo starego; Upewnij się, że przeczytanie [opcje dostępu do danych programu ASP.NET](https://msdn.microsoft.com/en-us/library/ms178359.aspx) i [opcje magazynu danych (kompilowanie praktyczne aplikacje w chmurze z systemu Windows Azure)](../aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/data-storage-options.md) pierwszy tak, aby nie pobieraj zbyt daleko do metody dostępu, która nie jest prawo dla danego scenariusza.
...
- [Uzyskiwanie dostępu do danych w programie Visual Studio](https://msdn.microsoft.com/en-us/library/wzabh8c4.aspx) (MSDN). Lista łączy podobną do tej zawartości mapy, ale z fokus w Visual Studio, a nie ASP.NET.
