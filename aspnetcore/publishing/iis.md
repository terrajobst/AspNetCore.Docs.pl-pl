...
*Web.config* pliku konfiguruje moduł platformy ASP.NET Core i udostępnia inne konfiguracji usług IIS. Tworzenie, przekształcanie i publikowanie *web.config* jest obsługiwany przez `Microsoft.NET.Sdk.Web`, który znajduje się w przypadku ustawienia SDK projektu w górnej części projektu (*.csproj*) pliku `<Project Sdk="Microsoft.NET.Sdk.Web">`. Aby zapobiec Przekształcanie element docelowy programu MSBuild z *web.config* plików, dodawanie  **\<IsTransformWebConfigDisabled >** pliku projektu z ustawieniem dla właściwości `true`:
...
W większości przypadków Włączanie rejestrowania aplikacji pomaga w rozwiązywaniu problemów z aplikacji lub zwrotnego serwera proxy. Zobacz [rejestrowanie](xref:fundamentals/logging) Aby uzyskać więcej informacji.
