...
* Jeśli aplikacja używa funkcji uwierzytelniania/autoryzacji, takich jak `AuthorizePage` lub `[Authorize]`, należy umieścić wywołanie do `UseAuthentication` i `UseAuthorization` **po** `UseRouting`.
* Jeśli aplikacja używa funkcji [CORS](xref:security/cors) , takich jak `[EnableCors]`, umieść `UseCors` dalej.
