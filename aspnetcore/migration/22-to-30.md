...
* `Newtonsoft.Json` MVC obsługują &ndash; obsługi funkcji MVC z `Newtonsoft.Json` jest teraz częścią [Microsoft. AspNetCore. MVC. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
Wartość domyślna dla ASP.NET Core to teraz [System. Text. JSON](/dotnet/api/system.text.json?view=netcore-3.0), który jest nowy w programie .net Core 3,0. Rozważ użycie `System.Text.Json`, jeśli jest to możliwe. Jest wysoka wydajność i nie wymaga dodatkowej zależności biblioteki. Jednak ponieważ `System.Text.Json` jest nowy, może nie mieć obecnie funkcji wymaganych przez aplikację.

Aplikacja może wymagać `Newtonsoft.Json` integracji, jeśli używa funkcji specyficznej dla `Newtonsoft.Json`, takiej jak JsonPatch lub Converters, lub jeśli [formatuje](xref:web-api/advanced/formatting) typy specyficzne dla `Newtonsoft.Json`.

Aby użyć Json.NET w projekcie SignalR ASP.NET Core 3,0, zobacz [Switch to Newtonsoft. JSON](#switch-to-newtonsoftjson) w tym dokumencie.
...
### <a name="systemruntimeserialization-attributes-not-supported"></a>Atrybuty System. Runtime. Serialization nie są obsługiwane

Atrybuty [[DataContract]](xref:System.Runtime.Serialization.DataContractAttribute) i [[DataMember]](xref:System.Runtime.Serialization.DataMemberAttribute) nie są obsługiwane w `System.Text.Json`. Aby uzyskać więcej informacji, zobacz [Obsługa system. Text. JSON](https://github.com/dotnet/corefx/issues/38758)problemu z usługą GitHub.
...
Jeśli używasz funkcji programu `Newtonsoft.Json`, które nie są obsługiwane w programie `System.Text.Json`, możesz przełączyć się z powrotem do `Newtonsoft.Json`:

1. Zainstaluj [pakiet Microsoft.AspNetCore.SignalR.Protocols. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson) pakiet NuGet.
