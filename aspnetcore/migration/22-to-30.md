...
* Ustaw [docelowe Moniker struktury (TFM)](/dotnet/standard/frameworks) do `netcoreapp3.0`:
...
* Aktualizacja `Version` atrybutu w pozostałych `<PackageReference>` elementy `Microsoft.AspNetCore.*` pakietów do bieżącej wersji zapoznawczej (na przykład `3.0.0-preview5-19227-01`).

  Jeśli nie ma wersji 3.0 pakietu, pakietu może zostały zaniechane w 3.0. Wiele z tych pakietów są częścią `Microsoft.AspNetCore.App` i nie powinien odwoływać się oddzielnie. Aby wstępną listę pakietów, które nie są już utworzone w 3.0, zobacz [rezygnują pakietów zestawy udostępnionych framework 3.0 (aspnet/AspNetCore #3756)](https://github.com/aspnet/AspNetCore/issues/3756). *Udostępnionej platformy* to zbiór zestawów ( *.dll* plików), są zainstalowane na komputerze i odwołuje się `Microsoft.AspNetCore.App`. Aby uzyskać więcej informacji, zobacz [udostępnionej platformy](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/).

* Zestawy dla kilku istotnych składników zostały usunięte z `Microsoft.AspNetCore.App` 3.0. Dodaj `<PackageReference>` elementy z pakiety wymienione w przypadku używania interfejsów API [zestawy usuwany z Microsoft.AspNetCore.App 3.0 (aspnet/AspNetCore #3755)](https://github.com/aspnet/AspNetCore/issues/3755).

  Usunięto przykładem składników:

  * `Microsoft.AspNet.WebApi.Client`
  * `Microsoft.EntityFrameworkCore`
  * `System.Data.SqlClient`

  Lista zestawów w wysyłki `Microsoft.AspNetCore.App` nie została sfinalizowana, a następnie zmieni się przed RTM w wersji 3.0.
  ...
  `ReadAsAsync` Metodę o nazwie w poprzednim kodzie znajduje się w `Microsoft.AspNet.WebApi.Client`. Zainstaluj [Microsoft.AspNet.WebApi.Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) pakietu NuGet, aby rozwiązać ten problem kompilacji 3.0.
...
* Aktualizacja `Startup.ConfigureServices` do wywołania `AddNewtonsoftJson`.
  ...
  `AddNewtonsoftJson` jest zgodny z nowych metod rejestracji usługi MVC:
  ...
  * `AddControllers`
  ...
  Ustawienia na składnik Json.NET można ustawić w wywołaniu `AddNewtonsoftJson`:
...
3\.0 kompilacja środowiska uruchomieniowego jest scenariusz zgłoszenie zgody na uczestnictwo. Aby włączyć kompilacja środowiska uruchomieniowego, zobacz https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilation.
