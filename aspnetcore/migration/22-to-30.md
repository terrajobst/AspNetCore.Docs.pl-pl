...
* Jeśli aplikacja bezpośrednio uzyskuje dostęp do `RouteData.Routers` wewnątrz MVC w celu przeanalizowania adresów URL, można je zastąpić przy użyciu [LinkParser. ParsePathByEndpointName](xref:Microsoft.AspNetCore.Routing.LinkParserEndpointNameAddressExtensions.ParsePathByEndpointName*). 
...
Zgodnie z opisem w dokumentacji dotyczącej [różnic między wcześniejszymi wersjami routingu](xref:fundamentals/routing#differences-from-earlier-versions-of-routing)istnieją pewne różnice dotyczące generowania linków (na przykład przy użyciu `Url.Link` i podobnych interfejsów API). Należą do nich:
