...
ASP.NET Core 3,0 usuwa niektóre zestawy, które wcześniej były częścią odwołania do pakietu `Microsoft.AspNetCore.App`. Aby nadal korzystać z funkcji dostarczonych przez te zestawy, należy odwołać się do 3,0 wersji odpowiednich pakietów:
...
* Obsługa formatowania i negocjacji zawartości dla `System.Net.HttpClient` &ndash; pakiet NuGet [Microsoft. ASPNET. WebApi. Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) oferuje przydatną rozszerzalność do `System.Net.HttpClient` z interfejsami API, takimi jak `ReadAsAsync`, `PostJsonAsync` itd.
...
Jeśli aplikacja używa `AuthorizeFilter` jako filtru globalnego w MVC, zalecamy refaktoryzację kodu w celu dostarczenia zasad w wywołaniu `AddAuthorization`.
...
Zasady można także dostosować. Podczas kompilowania w poprzednim przykładzie `DefaultPolicy` jest skonfigurowany do wymagania uwierzytelniania i określonego zakresu:
...
W przypadku aplikacji, które używają <xref:Microsoft.AspNetCore.TestHost.TestServer> bezpośrednio z [hostem ogólnym](xref:fundamentals/host/web-host), Utwórz `TestServer` na <xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder> w <xref:Microsoft.Extensions.Hosting.GenericHostWebHostBuilderExtensions.ConfigureWebHost*>:
...
* [Niektóre ustawienia domyślne SameSite pliku cookie zmieniły się na None (ASPNET/anonse #348)](https://github.com/aspnet/Announcements/issues/348).
...
Element addauthorization został przeniesiony do innego zestawu
