...
Jeśli Twoje rozwiązanie polega na pliku [Global. JSON](/dotnet/core/tools/global-json) , który będzie przeznaczony dla konkretnej wersji zestaw .NET Core SDK, zaktualizuj swoją właściwość `version` do wersji 3,0 zainstalowanej na maszynie:
...
* W `<ItemGroup>`:
  * `Microsoft.AspNetCore.App` został usunięty. Aby uzyskać więcej informacji, zobacz temat [Informacje o strukturze](#framework-reference) w tym dokumencie.
  * `Microsoft.AspNetCore.Razor.Design` został usunięty i na poniższej liście pakietów nie jest już generowany.
...
* Projekty przeznaczone dla `Microsoft.NET.Sdk` lub `Microsoft.NET.Sdk.Razor` SDK powinny dodawać jawne `FrameworkReference` do `Microsoft.AspNetCore.App`:
...
ASP.NET Core 3,0 usuwa niektóre zestawy, które wcześniej były częścią odwołania do pakietu `Microsoft.AspNetCore.App`. Aby wizualizować, które zestawy zostały usunięte, Porównaj te dwa udostępnione foldery struktury. Na przykład porównanie wersji 2.2.7 i 3.0.0:
...
* Obsługa formatowania i negocjacji zawartości dla `System.Net.HttpClient` &ndash; pakiet NuGet [Microsoft. ASPNET. WebApi. Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) oferuje przydatną rozszerzalność do `System.Net.HttpClient` z interfejsami API, takimi jak `ReadAsAsync` i `PostJsonAsync`.

* Kompilacja środowiska uruchomieniowego Razor &ndash; obsługa kompilowania widoków i stron środowiska uruchomieniowego w czasie wykonywania jest teraz częścią [Microsoft. AspNetCore. MVC. Razor. RuntimeCompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation).

* Obsługa &ndash; MVC `Newtonsoft.Json` (Json.NET) jest obsługiwana w przypadku używania MVC z `Newtonsoft.Json` jest teraz częścią [Microsoft. AspNetCore. MVC. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
* `IHostingEnvironment` do `IWebHostEnvironment`, zobacz [ten anons](https://github.com/dotnet/AspNetCore/issues/7749)w witrynie GitHub.
* `app.UseAuthorization` dodano do szablonów, aby pokazać oprogramowanie pośredniczące do zatwierdzania zamówień. Jeśli aplikacja nie korzysta z autoryzacji, możesz bezpiecznie usunąć wywołanie do `app.UseAuthorization`.
* `app.UseEndpoints`, zobacz [Razor Pages](#razor-pages) lub [Migrowanie uruchamiania. Skonfiguruj](#migrate-startupconfigure) w tym dokumencie.

### <a name="analyzer-support"></a>Obsługa analizatora

Projekty, które są przeznaczone dla niejawnie przywoływanych analizatorów odwołań, `Microsoft.NET.Sdk.Web` które zostały wcześniej wysłane jako część pakietu [Microsoft. AspNetCore. MVC. analizatory](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Analyzers/) . Nie są wymagane żadne dodatkowe odwołania, aby je włączyć.
...
Projekty są domyślne dla [modelu hostingu w procesie](xref:host-and-deploy/aspnet-core-module#in-process-hosting-model) w ASP.NET Core 3,0 lub nowszym. Opcjonalnie można usunąć Właściwość `<AspNetCoreHostingModel>` w pliku projektu, jeśli jej wartość jest `InProcess`.
...
W przypadku wersji 3,0 dostępne są następujące metody `RequestTrailerExtensions`:

* `GetDeclaredTrailers` &ndash; pobiera nagłówek `Trailer` żądania, który zawiera listę przyczep, które będą oczekiwać po treści.
* `SupportsTrailers` &ndash; wskazuje, czy żądanie obsługuje nagłówki przyczepy.
* `CheckTrailersAvailable` &ndash; sprawdza, czy żądanie obsługuje przyczepy i czy są dostępne do odczytu. To sprawdzenie nie zakłada, że istnieją przyczepy do odczytu. Być może nie ma żadnych przyczep do odczytania, nawet jeśli `true` jest zwracana przez tę metodę.
...
Jeśli jest wymagana synchroniczna operacja we/wy, można ją włączyć, konfigurując opcję `AllowSynchronousIO` na używanym serwerze (podczas wywoływania `ConfigureKestrel`, na przykład jeśli jest używany Kestrel). Należy zauważyć, że serwery (Kestrel, HttpSys, TestServer itp.) mają własną opcję `AllowSynchronousIO`, która nie ma wpływu na inne serwery. Synchroniczna operacja we/wy można włączyć dla wszystkich serwerów w poszczególnych żądaniach, korzystając z opcji `IHttpBodyControlFeature.AllowSynchronousIO`:
...
Jeśli masz problemy z implementacjami <xref:System.IO.TextWriter> lub innymi strumieniami wywołującymi synchroniczne interfejsy API w operacji [Dispose](/dotnet/standard/garbage-collection/implementing-dispose), zamiast tego wywołaj nowe <xref:System.IO.Stream.DisposeAsync*> API.
...
## <a name="newtonsoftjson-jsonnet-support"></a>Obsługa Newtonsoft. JSON (Json.NET)
...
Domyślny serializator JSON dla ASP.NET Core jest teraz <xref:System.Text.Json>, co nowego w programie .NET Core 3,0. Rozważ użycie `System.Text.Json`, jeśli jest to możliwe. Jest wysoka wydajność i nie wymaga dodatkowej zależności biblioteki. Jednak ponieważ `System.Text.Json` jest nowy, może nie mieć obecnie funkcji wymaganych przez aplikację. Aby uzyskać więcej informacji, zobacz [Migrowanie z Newtonsoft. JSON do System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to).

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-opno-locsignalr-project"></a>Używanie Newtonsoft. JSON w projekcie SignalR ASP.NET Core 3,0
...
* Na kliencie należy utworzyć łańcuch wywołania metody `AddNewtonsoftJsonProtocol` do wystąpienia `HubConnectionBuilder`:
...
* Na serwerze należy utworzyć łańcuch wywołania metody `AddNewtonsoftJsonProtocol` do wywołania metody `AddSignalR` w `Startup.ConfigureServices`:
...
### <a name="use-newtonsoftjson-in-an-aspnet-core-30-mvc-project"></a>Używanie Newtonsoft. JSON w projekcie 3,0 ASP.NET Core MVC
...
* Zaktualizuj `Startup.ConfigureServices`, aby wywołać `AddNewtonsoftJson`.
  ...
  * `AddControllers`
  ...
  Ustawienia `Newtonsoft.Json` można ustawić w wywołaniu `AddNewtonsoftJson`:
...
Dostępne są trzy nowe metody rozszerzenia najwyższego poziomu związane z scenariuszami MVC na `IServiceCollection`. Szablony wykorzystują te nowe metody zamiast `AddMvc`. Jednak `AddMvc` nadal działają tak, jak w poprzednich wersjach.
...
* Jeśli aplikacja bezpośrednio uzyskuje dostęp do `RouteData.Routers` wewnątrz MVC w celu przeanalizowania adresów URL, można to zastąpić przy użyciu `LinkParser.ParsePathByEndpointName`. 
 * Zdefiniuj trasę przy użyciu nazwy trasy.
 * Użyj `LinkParser.ParsePathByEndpointName` i przekaż odpowiednią nazwę trasy.
...
* Zastąp `UseMvc` lub `UseSignalR` `UseEndpoints`.
* Jeśli aplikacja używa scenariuszy [CORS](xref:security/cors) , takich jak `[EnableCors]`, należy umieścić wywołanie do `UseCors` przed innym oprogramowanie pośredniczące używające mechanizmu CORS (na przykład umieścić `UseCors` przed `UseAuthentication`, `UseAuthorization`i `UseEndpoints`).
* Zastąp `IHostingEnvironment` `IWebHostEnvironment` i Dodaj instrukcję `using` dla przestrzeni nazw <xref:Microsoft.Extensions.Hosting?displayProperty=fullName>.
* Zastąp `IApplicationLifetime` <xref:Microsoft.Extensions.Hosting.IHostApplicationLifetime> (<xref:Microsoft.Extensions.Hosting?displayProperty=fullName> przestrzeni nazw).
* Zastąp `EnvironmentName` <xref:Microsoft.Extensions.Hosting.Environments> (<xref:Microsoft.Extensions.Hosting?displayProperty=fullName> przestrzeni nazw).
...
Aby uzyskać więcej informacji, zobacz temat <xref:host-and-deploy/health-checks>.
...
W ASP.NET Core 3,0 lub nowszej `UseAuthorization` jest wywoływana w `Startup.Configure`i następujące `HomeController` wymagają zalogowanego użytkownika:
...
W przypadku korzystania z routingu punktów końcowych zalecamy skonfigurowanie `<xref:Microsoft.AspNetCore.Mvc.Authorization.AuthorizeFilter>` i zamianę polegania na oprogramowaniu pośredniczącym autoryzacji.  Jeśli aplikacja używa `AuthorizeFilter` jako filtru globalnego w MVC, zalecamy refaktoryzację kodu w celu dostarczenia zasad w wywołaniu `AddAuthorization`.

`DefaultPolicy` jest początkowo skonfigurowana do wymagania uwierzytelniania, dlatego nie jest wymagana żadna dodatkowa konfiguracja. W poniższym przykładzie punkty końcowe MVC są oznaczone jako `RequireAuthorization`, dzięki czemu wszystkie żądania muszą być autoryzowane na podstawie `DefaultPolicy`. Jednak `HomeController` zezwala na dostęp bez logowania użytkownika do aplikacji ze względu na `[AllowAnonymous]`:
...
Autoryzację można również skonfigurować dla określonych klas punktów końcowych. Poniższy kod stanowi przykład konwersji aplikacji MVC, która skonfigurowała `AuthorizeFilter` globalne do aplikacji z określonymi zasadami wymagającymi autoryzacji:

[!code-csharp[](22-to-30/samples/Startup.cs?name=snippet&highlight=8-9,21-22,26-27,53-54)]

Zasady można także dostosować. `DefaultPolicy` jest skonfigurowany do wymagania uwierzytelniania:
...
Alternatywnie można skonfigurować wszystkie punkty końcowe, aby wymagać autoryzacji bez `[Authorize]` lub `RequireAuthorization` przez skonfigurowanie `FallbackPolicy`. `FallbackPolicy` różni się od `DefaultPolicy`. `DefaultPolicy` jest wyzwalana przez `[Authorize]` lub `RequireAuthorization`, podczas gdy `FallbackPolicy` jest wyzwalana, gdy nie ustawiono żadnych innych zasad. `FallbackPolicy` jest początkowo skonfigurowana do zezwalania na żądania bez autoryzacji.

Poniższy przykład jest taki sam jak powyższy `DefaultPolicy` przykład, ale używa `FallbackPolicy`, aby zawsze wymagał uwierzytelniania we wszystkich punktach końcowych, z wyjątkiem sytuacji, gdy `[AllowAnonymous]` jest określony:
...
Ponadto każdy punkt końcowy może dostosować wymagania dotyczące autoryzacji. W poniższym przykładzie `UseAuthorization` przetwarza autoryzację przy użyciu `DefaultPolicy`, ale punkt końcowy sprawdzania kondycji `/healthz` wymaga `admin` użytkownika:
...
Jeśli aplikacja używa niestandardowych [programów obsługi autoryzacji](xref:security/authorization/policies#authorization-handlers), routing punktu końcowego przekazuje inny typ zasobu do obsługi niż MVC. Programy obsługi, które oczekują, że zasób kontekstu procedury obsługi autoryzacji ma typ <xref:Microsoft.AspNetCore.Mvc.Filters.AuthorizationFilterContext> (typ zasobu [dostarczony przez filtry MVC](xref:security/authorization/policies#accessing-mvc-request-context-in-handlers)) będzie musiał zostać zaktualizowany w celu obsługi zasobów typu <xref:Microsoft.AspNetCore.Routing.RouteEndpoint> (typ zasobu podany dla programów obsługi autoryzacji według routingu punktów końcowych).

MVC nadal używa zasobów `AuthorizationFilterContext`, więc jeśli aplikacja używa filtrów autoryzacji MVC wraz z autoryzacją routingu punktu końcowego, może być konieczne obsługę obu typów zasobów.

### SignalR

Mapowanie SignalR centrów odbywa się teraz w `UseEndpoints`.

Mapuj każde centrum za pomocą `MapHub`. Tak jak w poprzednich wersjach, każde centrum jest jawnie wymienione.

W poniższym przykładzie dodano obsługę centrum SignalR `ChatHub`:
...
Obecnie mapowanie kontrolerów odbywa się w `UseEndpoints`.
...
Zgodnie z opisem w dokumentacji dotyczącej [różnic między wcześniejszymi wersjami routingu](xref:fundamentals/routing#differences-from-earlier-versions-of-routing)istnieją pewne różnice dotyczące generowania linków (na przykład przy użyciu `Url.Link` i podobnych interfejsów API). Należą do nich następujące elementy:

* Domyślnie podczas korzystania z routingu punktów końcowych wielkość liter w parametrach trasy w wygenerowanych identyfikatorach URI nie musi być zachowywana. Takie zachowanie może być kontrolowane za pomocą interfejsu `IOutboundParameterTransformer`.
...
W poniższej metodzie `Startup.Configure` `MapRazorPages` dodaje obsługę Razor Pages:
...
Jeśli aplikacja wymaga obsługi starszej wersji `IRouter`, należy wyłączyć `EnableEndpointRouting` przy użyciu dowolnych z następujących metod w `Startup.ConfigureServices`:
...
Dodaj `MapHealthChecks`, aby użyć kontroli kondycji z routingiem punktów końcowych. Metoda `MapHealthChecks` akceptuje argumenty podobne do `UseHealthChecks`. Zaletą korzystania z `MapHealthChecks` za pośrednictwem `UseHealthChecks` jest możliwość stosowania autoryzacji i bardziej szczegółowej kontroli nad pasującymi zasadami.
...
Szablony ASP.NET Core 3,0 korzystają z [hosta ogólnego](xref:fundamentals/host/generic-host). Poprzednie wersje używają [hosta sieci Web](xref:fundamentals/host/web-host). Poniższy kod przedstawia wygenerowany `Program` klasy ASP.NET Core 3,0:

[!code-csharp[](22-to-30/samples/Program.cs?name=snippet)]

Poniższy kod przedstawia klasę `Program` wygenerowaną przez szablon ASP.NET Core 2,2:
...
Najbardziej znacząca zmiana z `WebHostBuilder` na `HostBuilder` polega na [iniekcji zależności (di)](xref:fundamentals/dependency-injection). W przypadku korzystania z `HostBuilder`można wstrzyknąć tylko następujące elementy do konstruktora `Startup`:
...
ASP.NET Core 2,2 i niższy `AddAuthorization` metody w *pliku Microsoft. AspNetCore. Authorization. dll*:
...
* Dodaj odwołanie do *pliku Microsoft. AspNetCore. Authorization. Policy. dll*. Ta metoda działa w przypadku większości aplikacji i jest wymagana.
* Przełącz do korzystania z `AddAuthorizationCore`

Aby uzyskać więcej informacji, zobacz artykuł dotyczący [zmiany w obciążeniu `AddAuthorization(o =>`) w innym #386 zestawu](https://github.com/aspnet/Announcements/issues/386).

## <a name="identity-ui"></a>Interfejs użytkownika tożsamości

Aktualizacje interfejsu użytkownika tożsamości dla ASP.NET Core 3,0:

* Dodaj odwołanie do pakietu do [Microsoft. AspNetCore. Identity. UI](https://nuget.org/packages/Microsoft.AspNetCore.Identity.UI).
* Aplikacje, które nie używają Razor Pages muszą wywoływać `MapRazorPages`. Zobacz [Razor Pages](#razor-pages) w tym dokumencie.
* Uruchomienie Bootstrap 4 jest domyślną strukturą interfejsu użytkownika. Ustaw właściwość projektu `IdentityUIFrameworkVersion`, aby zmienić wartość domyślną. Aby uzyskać więcej informacji, zobacz [ten anons usługi GitHub](https://github.com/aspnet/Announcements/issues/380).

## SignalR

Klient SignalR JavaScript zmienił się z `@aspnet/signalr` na `@microsoft/signalr`. Aby reagować na tę zmianę, Zmień odwołania w plikach *Package. JSON* , instrukcje `require` i instrukcje `import` języka ECMAScript.
...
Jeśli używasz [funkcji Newtonsoft. JSON, które nie są obsługiwane w pliku System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to), możesz przełączyć się z powrotem do `Newtonsoft.Json`. Zobacz [Używanie Newtonsoft. JSON w projekcie SignalR ASP.NET Core 3,0](#use-newtonsoftjson-in-an-aspnet-core-30-signalr-project) we wcześniejszej części tego artykułu.

## <a name="redis-distributed-caches"></a>Rozproszone pamięci podręczne Redis

Pakiet [Microsoft. Extensions. buforowania. Redis](https://www.nuget.org/packages/Microsoft.Extensions.Caching.Redis) nie jest dostępny dla aplikacji ASP.NET Core 3,0 lub nowszych. Zastąp odwołanie do pakietu pakietem [Microsoft. Extensions. buforowanie. StackExchangeRedis](https://www.nuget.org/packages/Microsoft.Extensions.Caching.StackExchangeRedis). Aby uzyskać więcej informacji, zobacz temat <xref:performance/caching/distributed>.
...
Przykład w https://github.com/aspnet/samples/tree/master/samples/aspnetcore/mvc/runtimecompilation przedstawia przykładowy sposób włączania kompilacji środowiska uruchomieniowego warunkowo w środowiskach programistycznych.
...
Aby uzyskać więcej informacji na temat używania ASP.NET Core interfejsów API w bibliotece klas, zobacz <xref:fundamentals/target-aspnetcore>.

## <a name="miscellaneous-changes"></a>Różne zmiany

System sprawdzania poprawności w programie .NET Core 3,0 lub nowszy traktuje parametry niedopuszczające wartości null lub właściwości powiązane tak, jakby miały atrybut `[Required]`. Aby uzyskać więcej informacji, zobacz [[Required] Attribute](xref:mvc/models/validation?view=aspnetcore-3.1#required-attribute).
...
W przypadku aplikacji, które używają <xref:Microsoft.AspNetCore.TestHost.TestServer> bezpośrednio z [hostem ogólnym](xref:fundamentals/host/web-host), Utwórz `TestServer` na <xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder> w <xref:Microsoft.Extensions.Hosting.GenericHostWebHostBuilderExtensions.ConfigureWebHost%2A>:
...
Aby uzyskać postęp wdrażania programu .NET Core do Azure App Service, zobacz oficjalny program [.NET Core w witrynie App Service](https://aspnetcoreon.azurewebsites.net/) . Dopóki program .NET Core 3,0 jest dostępny w Azure App Service, postępuj zgodnie z instrukcjami podanymi w tematach [wdrażanie ASP.NET Core wersja zapoznawcza do Azure App Service](xref:host-and-deploy/azure-apps/index#deploy-aspnet-core-preview-release-to-azure-app-service).
