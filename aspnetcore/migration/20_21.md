...
* Usuń atrybut "Version" w odwołaniu do pakietów do `Microsoft.AspNetCore.App`. Projekty używające `<Project Sdk="Microsoft.NET.Sdk.Web">` trzeba ustawić wersję. Wersja jest implikowana przez platformę docelową i wybrano, aby możliwie najlepiej odpowiadały działania platformy ASP.NET Core 2.1. Aby uzyskać więcej informacji, zobacz [reguły dla projektów przeznaczonych dla udostępnionego framework](#rules-for-projects-targeting-the-shared-framework) sekcji.
...
A *udostępnionej platformy* to zbiór zestawów ( *.dll* pliki) nie znajdują się w folderach aplikacji. Udostępnionej platformy musi być zainstalowany na komputerze, aby uruchomić aplikację. Aby uzyskać więcej informacji, zobacz [udostępnionej platformy](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/).
...
Znane problemy dotyczące projektów przeznaczonych dla udostępnionego framework:
...
* W *obszarów/tożsamość/stron/\_ViewStart.cshtml*, zmień `Layout = "/Pages/_Layout.cshtml"` do `Layout = "/Pages/Shared/_Layout.cshtml"`.
* *\_Layout.cshtml* plik zawiera następujące zmiany:
  ...
  * jQuery zmieni się z 2.2.0 3.3.1.
