...
## <a name="rules-for-projects-targeting-the-shared-runtime"></a>Reguły dla projektów przeznaczonych dla udostępnionego środowiska uruchomieniowego

Platforma ASP.NET Core obejmuje poniższe współużytkowane środowiska uruchomieniowe:
...
Znane problemy dotyczące projektów przeznaczonych dla udostępnionego środowiska uruchomieniowego:

* Za pomocą `Microsoft.AspNetCore.App` 2.1.0 przy użyciu platformy Entity Framework Core 2.1.1 powoduje niepowodzenia przywracania NuGet z powodu pakiet powoduje konflikt. Zalecanym rozwiązaniem jest uaktualnienie `Microsoft.AspNetCore.App` do 2.1.1. Aby uzyskać więcej informacji, zobacz [pakiety, które dzielą się zależnościami za pomocą Microsoft.AspNetCore.App nie odwołują się do wersji poprawki](https://github.com/aspnet/Universe/issues/1180).
...
Aplikacje, które mają kod przy użyciu tożsamości i Zastąp 2.0 tożsamości interfejsu użytkownika, za pomocą 2.1 biblioteki tożsamości trzeba wziąć pod uwagę adresy URL tożsamości ma `/Identity` się dołączony do identyfikatory URI. Jednym ze sposobów, aby obsługiwać nowe punkty końcowe tożsamości jest skonfigurowanie przekierowania, na przykład z `/Account/Login` do `/Identity/Account/Login`.
...
* Użyj kodu tożsamości interfejsu użytkownika w wersji 2.0, bez konieczności wprowadzania zmian. Przy użyciu tożsamości interfejsu użytkownika w wersji 2.0 kodu jest w pełni obsługiwany. Jest to dobra metoda, gdy wprowadzono zmiany znaczący wygenerowanego kodu tożsamości.
...
* Usuń Generator szkieletu tożsamości, generowane `IdentityDbContext` derrived klasy w *obszarów/tożsamość/Data/* folderu.
