...
Aby zastosować [model hostingu w procesie dla usług IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), należy dodać `<AspNetCoreHostingModel>` Właściwość `InProcess` o `<PropertyGroup>` wartości do w pliku projektu:
...
* `AspNetCoreModuleV2` `AspNetCoreModule` `modules` `name="aspNetCore"`W pozycji, która dodaje moduł ASP.NET Core (), Zmień wartość atrybutu z na. `<handlers>`
* W elemencie Dodaj atrybut model hostingu (`hostingModel="InProcess"`). `<aspNetCore>`
...
Jeśli celem jest .NET Core, Usuń `Version` atrybut odwołania pakietu w pliku projektu. Dołączenie `Version` atrybutu powoduje następujące ostrzeżenie:
...
W przypadku .NET Framework określania wartości docelowej należy zaktualizować `Version` atrybut odwołania każdego pakietu do 2.2.0 lub nowszego. Poniżej znajdują się odwołania do pakietów w typowym .NET Framework projektu ASP.NET Core 2,2:
...
W przypadku odwoływania się do pakietu [Microsoft. AspNetCore. Razor. Design](https://www.nuget.org/packages/Microsoft.AspNetCore.Razor.Design/) należy zaktualizować jego `Version` atrybut do 2.2.0 lub nowszego. Niewykonanie tej czynności spowoduje następujący błąd:
...
Jeśli Twoje rozwiązanie polega na pliku [Global. JSON](/dotnet/core/tools/global-json) , który będzie przeznaczony dla konkretnej wersji zestaw .NET Core SDK, zaktualizuj `version` jego właściwość do wersji 2,2 zainstalowanej na maszynie:
...
W przypadku używania Visual Studio Code należy zaktualizować plik ustawień uruchamiania projektu ( *. programu vscode/Launch. JSON*). `program` Ścieżka powinna odwoływać się do nowego TFM:

[!code-json[](21-to-22/samples/launch.json?highlight=9)]

## <a name="update-kestrel-configuration"></a>Aktualizuj konfigurację Kestrel

Jeśli aplikacja <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> wywołuje `UseKestrel` `ConfigureKestrel` [](xref:fundamentals/host/web-host#set-up-a-host) wywołanie metody `Program` CreateWebHostBuilder klasy, należy wywołać konfigurację serwera Kestrel, a nie w celu uniknięcia konfliktów `CreateDefaultBuilder` z [usługami IIS. Model hostingu w procesie](xref:host-and-deploy/iis/index#in-process-hosting-model):
...
`CreateDefaultBuilder` Jeśli aplikacja nie wywołuje i nie kompiluje hosta ręcznie `Program` w klasie, wywołaj <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **przed** wywołaniem `ConfigureKestrel`:
...
Zaktualizuj wersję zgodności w programie `Startup.ConfigureServices` do `Version_2_2`wersji:
...
Zmień linie w pliku dockerfile, aby użyć nowych tagów obrazu w kolumnie 2,2 poprzedniej tabeli. `FROM`
