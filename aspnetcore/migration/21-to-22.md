...
Jeśli aplikacja wywołuje <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> w `CreateDefaultBuilder` metody `Program` klasy, wywołaj `ConfigureKestrel` do skonfigurowania serwera Kestrel zamiast `UseKestrel` w celu uniknięcia konfliktów z [usług IIS w procesie modelu hostingu](xref:fundamentals/servers/index#in-process-hosting-model):
...
Jeśli aplikacja nie wywołać `CreateDefaultBuilder`, wywołaj <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **przed** wywoływania `ConfigureKestrel`:
