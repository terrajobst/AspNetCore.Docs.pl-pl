...
Projekty przeznaczone dla platformy .NET Core, należy użyć [TFM](/dotnet/standard/frameworks) wersji większy lub równy do platformy .NET Core w wersji 2.2. W pliku projektu, zaktualizuj `<TargetFramework>` węzła tekst wewnętrzny z `netcoreapp2.2`:
...
Przyjęcie [w trakcie modelu hostingu dla usług IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), Dodaj `<AspNetCoreHostingModel>` właściwość z wartością `InProcess` do `<PropertyGroup>` w pliku projektu:
...
## <a name="update-a-custom-webconfig-file"></a>Zaktualizuj plik web.config niestandardowe

Dla projektów, które korzystają z niestandardowego *web.config* plik w folderze głównym projektu do generowania ich opublikowanych *web.config* pliku:

* W `<handlers>` wpis, który dodaje modułu ASP.NET Core (`name="aspNetCore"`), zmień `modules` wartość z atrybutu `AspNetCoreModule` do `AspNetCoreModuleV2`.
* W `<aspNetCore>` elementu, Dodaj atrybut modelu hostingu (`hostingModel="InProcess"`).

Aby uzyskać więcej informacji i przykład *web.config* plików, zobacz <xref:host-and-deploy/aspnet-core-module#configuration-with-webconfig>.
...
Visual Studio **automatycznej kompilacji na żądanie przeglądarki** środowisko nie działa z [usług IIS w procesie modelu hostingu](xref:host-and-deploy/iis/index#in-process-hosting-model). Podczas korzystania z hostingu w procesie należy ręcznie ponownie skompilować projekt. Ulepszenia tego środowiska jest planowana w przyszłej wersji programu Visual Studio.
