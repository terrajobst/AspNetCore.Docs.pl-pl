...
Na powyższym obrazie segment adresu URL (`Parameters`) nie jest używany, parametry `name` i `numTimes` są przesyłane w [ciągu zapytania](https://wikipedia.org/wiki/Query_string). `?` (znak zapytania) w powyższym adresie URL jest separatorem, a ciąg zapytania znajduje się poniżej. Znak `&` oddziela pary pól i wartości.
