# <a name="key-vault-configuration-provider-sample-application-aspnet-core-1x"></a>Dostawca konfiguracji magazynu kluczy przykładowej aplikacji (platformy ASP.NET Core 1.x)

Ten przykład przedstawia użycie dostawcy konfiguracji magazynu kluczy Azure dla platformy ASP.NET Core 1.x. Dla przykładu 2.x platformy ASP.NET Core, zobacz [dostawcy konfiguracji magazynu kluczy przykładowej aplikacji (platformy ASP.NET Core 2.x)](https://github.com/aspnet/Docs/tree/master/aspnetcore/security/key-vault-configuration/samples/basic-sample/2.x).
