---
title: Strony ASP.NET Core Razor, EF podstawowych — samouczek serii
author: rick-anderson
description: ''
manager: wpickett
ms.author: riande
ms.date: 03/15/2017
ms.prod: asp.net-core
ms.technology: aspnet
ms.topic: get-started-article
uid: data/ef-rp/index
ms.openlocfilehash: 4247dff434b83658d18cd6d4bd22d6165258230e
ms.sourcegitcommit: 48beecfe749ddac52bc79aa3eb246a2dcdaa1862
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 03/22/2018
ms.locfileid: "30070423"
---
# <a name="aspnet-core-razor-pages-with-ef-core---tutorial-series"></a>Strony ASP.NET Core Razor, EF podstawowych — samouczek serii

Tej serii samouczków jest przedstawienie sposobu tworzenia aplikacji sieci web platformy ASP.NET Core Razor strony, które używają Core Entity Framework (EF) dla dostępu do danych. Samouczków wymagają programu Visual Studio 2017 r.

1. [Wprowadzenie](xref:data/ef-rp/intro)
1. [Operacje tworzenia, odczytu, aktualizowania i usuwania](xref:data/ef-rp/crud)
1. [Sortowanie, filtrowanie, stronicowanie i grupowanie](xref:data/ef-rp/sort-filter-page)
1. [Migracje](xref:data/ef-rp/migrations)
1. [Tworzenie złożonego modelu danych](xref:data/ef-rp/complex-data-model)
1. [Odczytywanie powiązanych danych](xref:data/ef-rp/read-related-data)
1. [Aktualizowanie powiązanych danych](xref:data/ef-rp/update-related-data)
1. [Obsługa konfliktów współbieżności](xref:data/ef-rp/concurrency)
