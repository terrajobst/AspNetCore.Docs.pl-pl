...
## <a name="security"></a><span data-ttu-id="d5a2e-145">Zabezpieczenia</span><span class="sxs-lookup"><span data-stu-id="d5a2e-145">Security</span></span>
...
<span data-ttu-id="d5a2e-157">W następującym pliku JSON cztery klucze, istnieją w ze strukturą hierarchii dwie sekcje:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-157">In the following JSON file, four keys exist in a structured hierarchy of two sections:</span></span>
...
<span data-ttu-id="d5a2e-168">Dostawcy konfiguracji pliku mają możliwość Załaduj ponownie konfigurację, gdy podstawowy plik ustawień zostanie zmieniona po uruchomieniu aplikacji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-168">File Configuration Providers have the ability to reload configuration when an underlying settings file is changed after app startup.</span></span> <span data-ttu-id="d5a2e-169">Dostawca konfiguracji pliku jest opisane w dalszej części tego tematu.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-169">The File Configuration Provider is described later in this topic.</span></span>
...
<span data-ttu-id="d5a2e-184">Wartości konfiguracji przyjęcie następujących konwencji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-184">Configuration values adopt the following conventions:</span></span>
...
1. <span data-ttu-id="d5a2e-241">Pliki (*appsettings.json*, *appsettings. { Środowisko} .json*, gdzie `{Environment}` jest bieżące środowisko hostingu aplikacji)</span><span class="sxs-lookup"><span data-stu-id="d5a2e-241">Files (*appsettings.json*, *appsettings.{Environment}.json*, where `{Environment}` is the app's current hosting environment)</span></span>
...
| <span data-ttu-id="d5a2e-316">Key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-316">Key</span></span>       | <span data-ttu-id="d5a2e-317">Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-317">Value</span></span>             |
| --------- | ----------------- |
| `-CLKey1` | `CommandLineKey1` |
| `-CLKey2` | `CommandLineKey2` |
...
| <span data-ttu-id="d5a2e-374">Prefiks ciągu połączenia</span><span class="sxs-lookup"><span data-stu-id="d5a2e-374">Connection string prefix</span></span> | <span data-ttu-id="d5a2e-375">Dostawcy</span><span class="sxs-lookup"><span data-stu-id="d5a2e-375">Provider</span></span> |
| ------------------------ | -------- |
...
### <a name="ini-configuration-provider"></a><span data-ttu-id="d5a2e-399">Dostawca konfiguracji INI</span><span class="sxs-lookup"><span data-stu-id="d5a2e-399">INI Configuration Provider</span></span>

<span data-ttu-id="d5a2e-400"><xref:Microsoft.Extensions.Configuration.Ini.IniConfigurationProvider> Ładuje konfiguracji z pary klucz wartość pliku INI w czasie wykonywania.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-400">The <xref:Microsoft.Extensions.Configuration.Ini.IniConfigurationProvider> loads configuration from INI file key-value pairs at runtime.</span></span>

<span data-ttu-id="d5a2e-401">Aby aktywować konfiguracji pliku INI, należy wywołać <xref:Microsoft.Extensions.Configuration.IniConfigurationExtensions.AddIniFile*> metody rozszerzenia w wystąpieniu <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-401">To activate INI file configuration, call the <xref:Microsoft.Extensions.Configuration.IniConfigurationExtensions.AddIniFile*> extension method on an instance of <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span></span>

<span data-ttu-id="d5a2e-402">Dwukropek może służyć do jako ogranicznik sekcji w konfiguracji pliku INI.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-402">The colon can be used to as a section delimiter in INI file configuration.</span></span>

<span data-ttu-id="d5a2e-403">Przeciążenia zezwala na określenie:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-403">Overloads permit specifying:</span></span>

* <span data-ttu-id="d5a2e-404">Czy plik jest opcjonalne.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-404">Whether the file is optional.</span></span>
* <span data-ttu-id="d5a2e-405">Czy konfiguracji jest załadowany ponownie, jeśli zmieni się plik.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-405">Whether the configuration is reloaded if the file changes.</span></span>
* <span data-ttu-id="d5a2e-406"><xref:Microsoft.Extensions.FileProviders.IFileProvider> Używane do uzyskania dostępu do pliku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-406">The <xref:Microsoft.Extensions.FileProviders.IFileProvider> used to access the file.</span></span>

::: moniker range=">= aspnetcore-2.1"

<span data-ttu-id="d5a2e-407">Wywołaj <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> podczas tworzenia hosta, aby określić konfigurację aplikacji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-407">Call <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> when building the host to specify the app's configuration:</span></span>
...
<span data-ttu-id="d5a2e-408">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-408">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="= aspnetcore-2.0"

<span data-ttu-id="d5a2e-409">Podczas wywoływania `CreateDefaultBuilder`, wywołaj <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-409">When calling `CreateDefaultBuilder`, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>
...
<span data-ttu-id="d5a2e-410">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-410">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="< aspnetcore-2.0"

<span data-ttu-id="d5a2e-411">Zastosuj konfigurację do <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> z <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> metody:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-411">Apply the configuration to <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> with the <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> method:</span></span>
...
<span data-ttu-id="d5a2e-412">Przykładowy plik konfiguracji INI:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-412">A generic example of an INI configuration file:</span></span>
...
<span data-ttu-id="d5a2e-413">Plik konfiguracyjny poprzedniego ładuje następujące klucze za pomocą `value`:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-413">The previous configuration file loads the following keys with `value`:</span></span>

* <span data-ttu-id="d5a2e-414">section0:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-414">section0:key0</span></span>
* <span data-ttu-id="d5a2e-415">section0:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-415">section0:key1</span></span>
* <span data-ttu-id="d5a2e-416">section1:Subsection:key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-416">section1:subsection:key</span></span>
* <span data-ttu-id="d5a2e-417">section2:subsection0:key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-417">section2:subsection0:key</span></span>
* <span data-ttu-id="d5a2e-418">section2:subsection1:key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-418">section2:subsection1:key</span></span>

### <a name="json-configuration-provider"></a><span data-ttu-id="d5a2e-419">Dostawca konfiguracji JSON</span><span class="sxs-lookup"><span data-stu-id="d5a2e-419">JSON Configuration Provider</span></span>

<span data-ttu-id="d5a2e-420"><xref:Microsoft.Extensions.Configuration.Json.JsonConfigurationProvider> Ładuje konfiguracji z pary klucz wartość plik JSON podczas wykonywania.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-420">The <xref:Microsoft.Extensions.Configuration.Json.JsonConfigurationProvider> loads configuration from JSON file key-value pairs during runtime.</span></span>

<span data-ttu-id="d5a2e-421">Aby aktywować konfiguracji pliku JSON, należy wywołać <xref:Microsoft.Extensions.Configuration.JsonConfigurationExtensions.AddJsonFile*> metody rozszerzenia w wystąpieniu <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-421">To activate JSON file configuration, call the <xref:Microsoft.Extensions.Configuration.JsonConfigurationExtensions.AddJsonFile*> extension method on an instance of <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span></span>

<span data-ttu-id="d5a2e-422">Przeciążenia zezwala na określenie:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-422">Overloads permit specifying:</span></span>

* <span data-ttu-id="d5a2e-423">Czy plik jest opcjonalne.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-423">Whether the file is optional.</span></span>
* <span data-ttu-id="d5a2e-424">Czy konfiguracji jest załadowany ponownie, jeśli zmieni się plik.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-424">Whether the configuration is reloaded if the file changes.</span></span>
* <span data-ttu-id="d5a2e-425"><xref:Microsoft.Extensions.FileProviders.IFileProvider> Używane do uzyskania dostępu do pliku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-425">The <xref:Microsoft.Extensions.FileProviders.IFileProvider> used to access the file.</span></span>

::: moniker range=">= aspnetcore-2.0"

<span data-ttu-id="d5a2e-426">`AddJsonFile` jest wywoływana automatycznie, dwa razy, podczas inicjowania nowego <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> z <xref:Microsoft.AspNetCore.WebHost.CreateDefaultBuilder*>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-426">`AddJsonFile` is automatically called twice when you initialize a new <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> with <xref:Microsoft.AspNetCore.WebHost.CreateDefaultBuilder*>.</span></span> <span data-ttu-id="d5a2e-427">Metoda jest wywoływana, aby załadować konfiguracji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-427">The method is called to load configuration from:</span></span>

* <span data-ttu-id="d5a2e-428">*appSettings.JSON* &ndash; ten plik jest najpierw do odczytu.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-428">*appsettings.json* &ndash; This file is read first.</span></span> <span data-ttu-id="d5a2e-429">Wersja środowiska pliku można zastąpić wartości dostarczone przez *appsettings.json* pliku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-429">The environment version of the file can override the values provided by the *appsettings.json* file.</span></span>
* <span data-ttu-id="d5a2e-430">*appSettings. {Środowiska} .json* &ndash; środowiska wersję pliku zostanie załadowany na podstawie [IHostingEnvironment.EnvironmentName](xref:Microsoft.Extensions.Hosting.IHostingEnvironment.EnvironmentName*).</span><span class="sxs-lookup"><span data-stu-id="d5a2e-430">*appsettings.{Environment}.json* &ndash; The environment version of the file is loaded based on the [IHostingEnvironment.EnvironmentName](xref:Microsoft.Extensions.Hosting.IHostingEnvironment.EnvironmentName*).</span></span>

<span data-ttu-id="d5a2e-431">Aby uzyskać więcej informacji, zobacz [hosta sieci Web: Konfigurowanie hosta](xref:fundamentals/host/web-host#set-up-a-host).</span><span class="sxs-lookup"><span data-stu-id="d5a2e-431">For more information, see [Web Host: Set up a host](xref:fundamentals/host/web-host#set-up-a-host).</span></span>

<span data-ttu-id="d5a2e-432">`CreateDefaultBuilder` także obciążenia:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-432">`CreateDefaultBuilder` also loads:</span></span>

* <span data-ttu-id="d5a2e-433">Zmienne środowiskowe.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-433">Environment variables.</span></span>
* <span data-ttu-id="d5a2e-434">[Wpisami tajnymi użytkowników (klucz tajny Menedżer)](xref:security/app-secrets) (w środowisku programistycznym).</span><span class="sxs-lookup"><span data-stu-id="d5a2e-434">[User secrets (Secret Manager)](xref:security/app-secrets) (in the Development environment).</span></span>
* <span data-ttu-id="d5a2e-435">Argumenty wiersza polecenia.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-435">Command-line arguments.</span></span>

<span data-ttu-id="d5a2e-436">Dostawca konfiguracji JSON jest ustanawiane, najpierw.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-436">The JSON Configuration Provider is established first.</span></span> <span data-ttu-id="d5a2e-437">W związku z tym, wpisami tajnymi użytkowników, zmienne środowiskowe i argumenty wiersza polecenia zastępuje konfiguracji ustawione przez *appsettings* plików.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-437">Therefore, user secrets, environment variables, and command-line arguments override configuration set by the *appsettings* files.</span></span>

::: moniker-end

::: moniker range=">= aspnetcore-2.1"

<span data-ttu-id="d5a2e-438">Wywołaj <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> podczas tworzenia hosta, aby określić konfigurację aplikacji dla plików innych niż *appsettings.json* i *appsettings. { Środowisko} .json*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-438">Call <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> when building the host to specify the app's configuration for files other than *appsettings.json* and *appsettings.{Environment}.json*:</span></span>
...
<span data-ttu-id="d5a2e-439">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-439">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="= aspnetcore-2.0"

<span data-ttu-id="d5a2e-440">Możesz również bezpośrednio wywołać `AddJsonFile` metody rozszerzenia w wystąpieniu <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-440">You can also directly call the `AddJsonFile` extension method on an instance of <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span></span>

<span data-ttu-id="d5a2e-441">Podczas wywoływania `CreateDefaultBuilder`, wywołaj <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-441">When calling `CreateDefaultBuilder`, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>
...
<span data-ttu-id="d5a2e-442">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-442">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="< aspnetcore-2.0"

<span data-ttu-id="d5a2e-443">Zastosuj konfigurację do <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> z <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> metody:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-443">Apply the configuration to <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> with the <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> method:</span></span>
...
<span data-ttu-id="d5a2e-444">**Przykład**</span><span class="sxs-lookup"><span data-stu-id="d5a2e-444">**Example**</span></span>

::: moniker range=">= aspnetcore-2.0"

<span data-ttu-id="d5a2e-445">2.x Przykładowa aplikacja korzysta z metody statyczne wygody `CreateDefaultBuilder` do hosta, który obejmuje dwa wywołania do tworzenia `AddJsonFile`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-445">The 2.x sample app takes advantage of the static convenience method `CreateDefaultBuilder` to build the host, which includes two calls to `AddJsonFile`.</span></span> <span data-ttu-id="d5a2e-446">Konfiguracja została załadowana z *appsettings.json* i *appsettings. { Środowisko} .json*.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-446">Configuration is loaded from *appsettings.json* and *appsettings.{Environment}.json*.</span></span>

::: moniker-end

::: moniker range="< aspnetcore-2.0"

<span data-ttu-id="d5a2e-447">Wywołania aplikacji przykładowej 1.x `AddJsonFile` dwa razy na `ConfigurationBuilder`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-447">The 1.x sample app calls `AddJsonFile` twice on a `ConfigurationBuilder`.</span></span> <span data-ttu-id="d5a2e-448">Konfiguracja została załadowana z *appsettings.json* i *appsettings. { Środowisko} .json*.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-448">Configuration is loaded from *appsettings.json* and *appsettings.{Environment}.json*.</span></span>

::: moniker-end

1. <span data-ttu-id="d5a2e-449">Uruchom przykładową aplikację.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-449">Run the sample app.</span></span> <span data-ttu-id="d5a2e-450">Otwórz przeglądarkę dla aplikacji w momencie `http://localhost:5000`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-450">Open a browser to the app at `http://localhost:5000`.</span></span>
1. <span data-ttu-id="d5a2e-451">Sprawdź, czy dane wyjściowe zawierają pary klucz wartość dla konfiguracji, jak pokazano w tabeli, w zależności od środowiska.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-451">Observe that the output contains key-value pairs for the configuration shown in the table depending on the environment.</span></span> <span data-ttu-id="d5a2e-452">Klucze konfiguracji rejestrowania, użyj dwukropka (`:`) jako separator hierarchicznej.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-452">Logging configuration keys use the colon (`:`) as a hierarchical separator.</span></span>

| <span data-ttu-id="d5a2e-453">Key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-453">Key</span></span>                        | <span data-ttu-id="d5a2e-454">Wartość rozwoju</span><span class="sxs-lookup"><span data-stu-id="d5a2e-454">Development Value</span></span> | <span data-ttu-id="d5a2e-455">Wartość produkcji</span><span class="sxs-lookup"><span data-stu-id="d5a2e-455">Production Value</span></span> |
| -------------------------- | :---------------: | :--------------: |
| <span data-ttu-id="d5a2e-456">Rejestrowanie: LogLevel:System</span><span class="sxs-lookup"><span data-stu-id="d5a2e-456">Logging:LogLevel:System</span></span>    | <span data-ttu-id="d5a2e-457">Informacje</span><span class="sxs-lookup"><span data-stu-id="d5a2e-457">Information</span></span>       | <span data-ttu-id="d5a2e-458">Informacje</span><span class="sxs-lookup"><span data-stu-id="d5a2e-458">Information</span></span>      |
| <span data-ttu-id="d5a2e-459">Rejestrowanie: LogLevel:Microsoft</span><span class="sxs-lookup"><span data-stu-id="d5a2e-459">Logging:LogLevel:Microsoft</span></span> | <span data-ttu-id="d5a2e-460">Informacje</span><span class="sxs-lookup"><span data-stu-id="d5a2e-460">Information</span></span>       | <span data-ttu-id="d5a2e-461">Informacje</span><span class="sxs-lookup"><span data-stu-id="d5a2e-461">Information</span></span>      |
| <span data-ttu-id="d5a2e-462">Rejestrowanie: LogLevel:Default</span><span class="sxs-lookup"><span data-stu-id="d5a2e-462">Logging:LogLevel:Default</span></span>   | <span data-ttu-id="d5a2e-463">Debugowanie</span><span class="sxs-lookup"><span data-stu-id="d5a2e-463">Debug</span></span>             | <span data-ttu-id="d5a2e-464">Błąd</span><span class="sxs-lookup"><span data-stu-id="d5a2e-464">Error</span></span>            |
| <span data-ttu-id="d5a2e-465">AllowedHosts</span><span class="sxs-lookup"><span data-stu-id="d5a2e-465">AllowedHosts</span></span>               | *                 | *                |

### <a name="xml-configuration-provider"></a><span data-ttu-id="d5a2e-466">Dostawca konfiguracji XML</span><span class="sxs-lookup"><span data-stu-id="d5a2e-466">XML Configuration Provider</span></span>

<span data-ttu-id="d5a2e-467"><xref:Microsoft.Extensions.Configuration.Xml.XmlConfigurationProvider> Ładuje konfiguracji z pary klucz wartość plik XML w czasie wykonywania.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-467">The <xref:Microsoft.Extensions.Configuration.Xml.XmlConfigurationProvider> loads configuration from XML file key-value pairs at runtime.</span></span>

<span data-ttu-id="d5a2e-468">Aby aktywować Konfiguracja pliku XML, należy wywołać <xref:Microsoft.Extensions.Configuration.XmlConfigurationExtensions.AddXmlFile*> metody rozszerzenia w wystąpieniu <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-468">To activate XML file configuration, call the <xref:Microsoft.Extensions.Configuration.XmlConfigurationExtensions.AddXmlFile*> extension method on an instance of <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span></span>

<span data-ttu-id="d5a2e-469">Przeciążenia zezwala na określenie:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-469">Overloads permit specifying:</span></span>

* <span data-ttu-id="d5a2e-470">Czy plik jest opcjonalne.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-470">Whether the file is optional.</span></span>
* <span data-ttu-id="d5a2e-471">Czy konfiguracji jest załadowany ponownie, jeśli zmieni się plik.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-471">Whether the configuration is reloaded if the file changes.</span></span>
* <span data-ttu-id="d5a2e-472"><xref:Microsoft.Extensions.FileProviders.IFileProvider> Używane do uzyskania dostępu do pliku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-472">The <xref:Microsoft.Extensions.FileProviders.IFileProvider> used to access the file.</span></span>

<span data-ttu-id="d5a2e-473">Węzeł główny plik konfiguracji jest ignorowany podczas tworzenia pary klucz wartość konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-473">The root node of the configuration file is ignored when the configuration key-value pairs are created.</span></span> <span data-ttu-id="d5a2e-474">Określać definicji typu dokumentu (DTD) lub przestrzeni nazw w pliku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-474">Don't specify a Document Type Definition (DTD) or namespace in the file.</span></span>

::: moniker range=">= aspnetcore-2.1"

<span data-ttu-id="d5a2e-475">Wywołaj <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> podczas tworzenia hosta, aby określić konfigurację aplikacji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-475">Call <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> when building the host to specify the app's configuration:</span></span>
...
<span data-ttu-id="d5a2e-476">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-476">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="= aspnetcore-2.0"

<span data-ttu-id="d5a2e-477">Podczas wywoływania `CreateDefaultBuilder`, wywołaj <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-477">When calling `CreateDefaultBuilder`, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>
...
<span data-ttu-id="d5a2e-478">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-478">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="< aspnetcore-2.0"

<span data-ttu-id="d5a2e-479">Zastosuj konfigurację do <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> z <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> metody:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-479">Apply the configuration to <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> with the <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> method:</span></span>
...
<span data-ttu-id="d5a2e-480">Pliki konfiguracji XML można użyć nazw unikatowych elementów dla powtarzających się sekcje:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-480">XML configuration files can use distinct element names for repeating sections:</span></span>
...
<span data-ttu-id="d5a2e-481">Plik konfiguracyjny poprzedniego ładuje następujące klucze za pomocą `value`:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-481">The previous configuration file loads the following keys with `value`:</span></span>

* <span data-ttu-id="d5a2e-482">section0:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-482">section0:key0</span></span>
* <span data-ttu-id="d5a2e-483">section0:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-483">section0:key1</span></span>
* <span data-ttu-id="d5a2e-484">section1:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-484">section1:key0</span></span>
* <span data-ttu-id="d5a2e-485">section1:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-485">section1:key1</span></span>

<span data-ttu-id="d5a2e-486">Powtarzające się elementy, które używają takiej samej nazwie elementu pracy, jeśli `name` atrybut jest używany do odróżniania elementów:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-486">Repeating elements that use the same element name work if the `name` attribute is used to distinguish the elements:</span></span>
...
<span data-ttu-id="d5a2e-487">Plik konfiguracyjny poprzedniego ładuje następujące klucze za pomocą `value`:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-487">The previous configuration file loads the following keys with `value`:</span></span>

* <span data-ttu-id="d5a2e-488">sekcja: section0:key:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-488">section:section0:key:key0</span></span>
* <span data-ttu-id="d5a2e-489">sekcja: section0:key:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-489">section:section0:key:key1</span></span>
* <span data-ttu-id="d5a2e-490">sekcja: section1:key:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-490">section:section1:key:key0</span></span>
* <span data-ttu-id="d5a2e-491">sekcja: section1:key:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-491">section:section1:key:key1</span></span>

<span data-ttu-id="d5a2e-492">Atrybuty można podać wartości:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-492">Attributes can be used to supply values:</span></span>
...
<span data-ttu-id="d5a2e-493">Plik konfiguracyjny poprzedniego ładuje następujące klucze za pomocą `value`:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-493">The previous configuration file loads the following keys with `value`:</span></span>

* <span data-ttu-id="d5a2e-494">klucz: atrybut</span><span class="sxs-lookup"><span data-stu-id="d5a2e-494">key:attribute</span></span>
* <span data-ttu-id="d5a2e-495">sekcja: klucz: atrybut</span><span class="sxs-lookup"><span data-stu-id="d5a2e-495">section:key:attribute</span></span>

::: moniker range=">= aspnetcore-2.1"

## <a name="key-per-file-configuration-provider"></a><span data-ttu-id="d5a2e-496">Dostawca klucza każdego pliku konfiguracji</span><span class="sxs-lookup"><span data-stu-id="d5a2e-496">Key-per-file Configuration Provider</span></span>

<span data-ttu-id="d5a2e-497"><xref:Microsoft.Extensions.Configuration.KeyPerFile.KeyPerFileConfigurationProvider> Korzysta z plików w katalogu jako pary klucz wartość konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-497">The <xref:Microsoft.Extensions.Configuration.KeyPerFile.KeyPerFileConfigurationProvider> uses a directory's files as configuration key-value pairs.</span></span> <span data-ttu-id="d5a2e-498">Klucz jest nazwą pliku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-498">The key is the file name.</span></span> <span data-ttu-id="d5a2e-499">Wartość zawiera zawartość pliku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-499">The value contains the file's contents.</span></span> <span data-ttu-id="d5a2e-500">Dostawca konfiguracji klucza każdego pliku jest używany w scenariuszach hostingu platformy Docker.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-500">The Key-per-file Configuration Provider is used in Docker hosting scenarios.</span></span>

<span data-ttu-id="d5a2e-501">Aby aktywować klucza dla pliku konfiguracji, należy wywołać <xref:Microsoft.Extensions.Configuration.KeyPerFileConfigurationBuilderExtensions.AddKeyPerFile*> metody rozszerzenia w wystąpieniu <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-501">To activate key-per-file configuration, call the <xref:Microsoft.Extensions.Configuration.KeyPerFileConfigurationBuilderExtensions.AddKeyPerFile*> extension method on an instance of <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span></span> <span data-ttu-id="d5a2e-502">`directoryPath` Do plików musi być ścieżką bezwzględną.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-502">The `directoryPath` to the files must be an absolute path.</span></span>

<span data-ttu-id="d5a2e-503">Przeciążenia zezwala na określenie:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-503">Overloads permit specifying:</span></span>

* <span data-ttu-id="d5a2e-504">`Action<KeyPerFileConfigurationSource>` Delegat, który konfiguruje źródło.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-504">An `Action<KeyPerFileConfigurationSource>` delegate that configures the source.</span></span>
* <span data-ttu-id="d5a2e-505">Czy katalog jest opcjonalna i ścieżkę do katalogu.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-505">Whether the directory is optional and the path to the directory.</span></span>

<span data-ttu-id="d5a2e-506">Wywołaj <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> podczas tworzenia hosta, aby określić konfigurację aplikacji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-506">Call <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> when building the host to specify the app's configuration:</span></span>
...
<span data-ttu-id="d5a2e-507">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-507">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>
...
## <a name="memory-configuration-provider"></a><span data-ttu-id="d5a2e-508">Dostawca konfiguracji pamięci</span><span class="sxs-lookup"><span data-stu-id="d5a2e-508">Memory Configuration Provider</span></span>

<span data-ttu-id="d5a2e-509"><xref:Microsoft.Extensions.Configuration.Memory.MemoryConfigurationProvider> Korzysta z kolekcji w pamięci jako pary klucz wartość konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-509">The <xref:Microsoft.Extensions.Configuration.Memory.MemoryConfigurationProvider> uses an in-memory collection as configuration key-value pairs.</span></span>

<span data-ttu-id="d5a2e-510">Aby aktywować konfiguracji kolekcji w pamięci, należy wywołać <xref:Microsoft.Extensions.Configuration.MemoryConfigurationBuilderExtensions.AddInMemoryCollection*> metody rozszerzenia w wystąpieniu <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-510">To activate in-memory collection configuration, call the <xref:Microsoft.Extensions.Configuration.MemoryConfigurationBuilderExtensions.AddInMemoryCollection*> extension method on an instance of <xref:Microsoft.Extensions.Configuration.ConfigurationBuilder>.</span></span>

<span data-ttu-id="d5a2e-511">Dostawca konfiguracji mogą być zainicjowane z `IEnumerable<KeyValuePair<String,String>>`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-511">The configuration provider can be initialized with an `IEnumerable<KeyValuePair<String,String>>`.</span></span>

::: moniker range=">= aspnetcore-2.1"

<span data-ttu-id="d5a2e-512">Wywołaj <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> podczas tworzenia hosta, aby określić konfigurację aplikacji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-512">Call <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*> when building the host to specify the app's configuration:</span></span>
...
<span data-ttu-id="d5a2e-513">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-513">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="= aspnetcore-2.0"

<span data-ttu-id="d5a2e-514">Podczas wywoływania `CreateDefaultBuilder`, wywołaj <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-514">When calling `CreateDefaultBuilder`, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>
...
<span data-ttu-id="d5a2e-515">Podczas tworzenia <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> , dzwoniąc <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> z konfiguracją:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-515">When creating a <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> directly, call <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> with the configuration:</span></span>

::: moniker-end

::: moniker range="< aspnetcore-2.0"

<span data-ttu-id="d5a2e-516">Zastosuj konfigurację do <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> z <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> metody:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-516">Apply the configuration to <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> with the <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseConfiguration*> method:</span></span>
...
## <a name="getvalue"></a><span data-ttu-id="d5a2e-517">GetValue</span><span class="sxs-lookup"><span data-stu-id="d5a2e-517">GetValue</span></span>

<span data-ttu-id="d5a2e-518">[ConfigurationBinder.GetValue&lt;T&gt; ](xref:Microsoft.Extensions.Configuration.ConfigurationBinder.GetValue*) wyodrębnianie wartości z konfiguracji z określonym kluczem i konwertuje je do określonego typu.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-518">[ConfigurationBinder.GetValue&lt;T&gt;](xref:Microsoft.Extensions.Configuration.ConfigurationBinder.GetValue*) extracts a value from configuration with a specified key and converts it to the specified type.</span></span> <span data-ttu-id="d5a2e-519">Przeciążenie umożliwia podanie wartości domyślnej, jeśli nie odnaleziono klucza.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-519">An overload permits you to provide a default value if the key isn't found.</span></span>

<span data-ttu-id="d5a2e-520">Poniższy przykład wyodrębnia wartość ciągu z konfiguracji przy użyciu klucza `NumberKey`, typy wartości jako `int`i przechowuje wartość w zmiennej `intValue`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-520">The following example extracts the string value from configuration with the key `NumberKey`, types the value as an `int`, and stores the value in the variable `intValue`.</span></span> <span data-ttu-id="d5a2e-521">Jeśli `NumberKey` nie zostanie odnaleziona w klucze konfiguracji `intValue` odbiera wartość domyślną `99`:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-521">If `NumberKey` isn't found in the configuration keys, `intValue` receives the default value of `99`:</span></span>

```csharp
var intValue = config.GetValue<int>("NumberKey", 99);
```

## <a name="getsection-getchildren-and-exists"></a><span data-ttu-id="d5a2e-522">GetSection, GetChildren i istnieje</span><span class="sxs-lookup"><span data-stu-id="d5a2e-522">GetSection, GetChildren, and Exists</span></span>

<span data-ttu-id="d5a2e-523">Aby uzyskać przykłady, które należy wykonać należy wziąć pod uwagę następujący plik JSON.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-523">For the examples that follow, consider the following JSON file.</span></span> <span data-ttu-id="d5a2e-524">Cztery klucze znajdują się na dwie sekcje, z których jedna zawiera parę podsekcje:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-524">Four keys are found across two sections, one of which includes a pair of subsections:</span></span>
...
<span data-ttu-id="d5a2e-525">Gdy plik jest do odczytu do konfiguracji, następujące klucze unikatowe hierarchiczne są tworzone do przechowywania wartości konfiguracji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-525">When the file is read into configuration, the following unique hierarchical keys are created to hold the configuration values:</span></span>

* <span data-ttu-id="d5a2e-526">section0:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-526">section0:key0</span></span>
* <span data-ttu-id="d5a2e-527">section0:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-527">section0:key1</span></span>
* <span data-ttu-id="d5a2e-528">section1:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-528">section1:key0</span></span>
* <span data-ttu-id="d5a2e-529">section1:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-529">section1:key1</span></span>
* <span data-ttu-id="d5a2e-530">section2:subsection0:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-530">section2:subsection0:key0</span></span>
* <span data-ttu-id="d5a2e-531">section2:subsection0:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-531">section2:subsection0:key1</span></span>
* <span data-ttu-id="d5a2e-532">section2:subsection1:key0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-532">section2:subsection1:key0</span></span>
* <span data-ttu-id="d5a2e-533">section2:subsection1:key1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-533">section2:subsection1:key1</span></span>

### <a name="getsection"></a><span data-ttu-id="d5a2e-534">GetSection</span><span class="sxs-lookup"><span data-stu-id="d5a2e-534">GetSection</span></span>

<span data-ttu-id="d5a2e-535">[IConfiguration.GetSection](xref:Microsoft.Extensions.Configuration.IConfiguration.GetSection*) wyodrębnia podsekcji konfiguracji z kluczem określonym podsekcji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-535">[IConfiguration.GetSection](xref:Microsoft.Extensions.Configuration.IConfiguration.GetSection*) extracts a configuration subsection with the specified subsection key.</span></span>

<span data-ttu-id="d5a2e-536">Aby zwrócić <xref:Microsoft.Extensions.Configuration.IConfigurationSection> zawierający tylko pary klucz wartość w `section1`, wywołaj `GetSection` i podaj nazwę sekcji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-536">To return an <xref:Microsoft.Extensions.Configuration.IConfigurationSection> containing only the key-value pairs in `section1`, call `GetSection` and supply the section name:</span></span>

```csharp
var configSection = _config.GetSection("section1");
```

<span data-ttu-id="d5a2e-537">Podobnie Aby uzyskać wartości kluczy w `section2:subsection0`, wywołaj `GetSection` i podaj ścieżkę do sekcji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-537">Similarly, to obtain the values for keys in `section2:subsection0`, call `GetSection` and supply the section path:</span></span>

```csharp
var configSection = _config.GetSection("section2:subsection0");
```

<span data-ttu-id="d5a2e-538">`GetSection` nigdy nie zwraca `null`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-538">`GetSection` never returns `null`.</span></span> <span data-ttu-id="d5a2e-539">Jeśli nie zostanie znaleziona pasująca sekcja, pusta `IConfigurationSection` jest zwracana.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-539">If a matching section isn't found, an empty `IConfigurationSection` is returned.</span></span>

### <a name="getchildren"></a><span data-ttu-id="d5a2e-540">GetChildren —</span><span class="sxs-lookup"><span data-stu-id="d5a2e-540">GetChildren</span></span>

<span data-ttu-id="d5a2e-541">Wywołanie [IConfiguration.GetChildren](xref:Microsoft.Extensions.Configuration.IConfiguration.GetChildren*) na `section2` uzyskuje `IEnumerable<IConfigurationSection>` zawierającej:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-541">A call to [IConfiguration.GetChildren](xref:Microsoft.Extensions.Configuration.IConfiguration.GetChildren*) on `section2` obtains an `IEnumerable<IConfigurationSection>` that includes:</span></span>
...
### <a name="exists"></a><span data-ttu-id="d5a2e-542">Istnieje</span><span class="sxs-lookup"><span data-stu-id="d5a2e-542">Exists</span></span>

<span data-ttu-id="d5a2e-543">Użyj [ConfigurationExtensions.Exists](xref:Microsoft.Extensions.Configuration.ConfigurationExtensions.Exists*) do ustalenia, czy istnieje sekcji konfiguracji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-543">Use [ConfigurationExtensions.Exists](xref:Microsoft.Extensions.Configuration.ConfigurationExtensions.Exists*) to determine if a configuration section exists:</span></span>

```csharp
var sectionExists = _config.GetSection("section2:subsection2").Exists();
```

<span data-ttu-id="d5a2e-544">Podane dane przykładowe `sectionExists` jest `false` , ponieważ nie ma `section2:subsection2` sekcji w danych konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-544">Given the example data, `sectionExists` is `false` because there isn't a `section2:subsection2` section in the configuration data.</span></span>

::: moniker-end

## <a name="bind-to-a-class"></a><span data-ttu-id="d5a2e-545">Powiąż z klasy</span><span class="sxs-lookup"><span data-stu-id="d5a2e-545">Bind to a class</span></span>

<span data-ttu-id="d5a2e-546">Konfiguracja może być powiązana z klas, które reprezentują grupy powiązane ustawienia za pomocą *wzorzec opcje*.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-546">Configuration can be bound to classes that represent groups of related settings using the *options pattern*.</span></span> <span data-ttu-id="d5a2e-547">Aby uzyskać więcej informacji, zobacz <xref:fundamentals/configuration/options>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-547">For more information, see <xref:fundamentals/configuration/options>.</span></span>

<span data-ttu-id="d5a2e-548">Wartości konfiguracji są zwracane jako ciągi, ale wywoływania <xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Bind*> umożliwia konstruowanie [POCO](https://wikipedia.org/wiki/Plain_Old_CLR_Object) obiektów.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-548">Configuration values are returned as strings, but calling <xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Bind*> enables the construction of [POCO](https://wikipedia.org/wiki/Plain_Old_CLR_Object) objects.</span></span>

<span data-ttu-id="d5a2e-549">Przykładowa aplikacja zawiera `Starship` modelu (*Models/Starship.cs*):</span><span class="sxs-lookup"><span data-stu-id="d5a2e-549">The sample app contains a `Starship` model (*Models/Starship.cs*):</span></span>
...
<span data-ttu-id="d5a2e-550">`starship` Części *starship.json* plików umożliwia utworzenie konfiguracji podczas Przykładowa aplikacja używa dostawcy konfiguracji JSON można załadować konfiguracji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-550">The `starship` section of the *starship.json* file creates the configuration when the sample app uses the JSON Configuration Provider to load the configuration:</span></span>
...
<span data-ttu-id="d5a2e-551">Tworzone są następujące pary klucz wartość konfiguracji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-551">The following configuration key-value pairs are created:</span></span>

| <span data-ttu-id="d5a2e-552">Key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-552">Key</span></span>                   | <span data-ttu-id="d5a2e-553">Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-553">Value</span></span>                                             |
| --------------------- | ------------------------------------------------- |
| <span data-ttu-id="d5a2e-554">starship: Nazwa</span><span class="sxs-lookup"><span data-stu-id="d5a2e-554">starship:name</span></span>         | <span data-ttu-id="d5a2e-555">USS przedsiębiorstwa</span><span class="sxs-lookup"><span data-stu-id="d5a2e-555">USS Enterprise</span></span>                                    |
| <span data-ttu-id="d5a2e-556">starship: rejestru</span><span class="sxs-lookup"><span data-stu-id="d5a2e-556">starship:registry</span></span>     | <span data-ttu-id="d5a2e-557">KCF 1701</span><span class="sxs-lookup"><span data-stu-id="d5a2e-557">NCC-1701</span></span>                                          |
| <span data-ttu-id="d5a2e-558">starship: klasy</span><span class="sxs-lookup"><span data-stu-id="d5a2e-558">starship:class</span></span>        | <span data-ttu-id="d5a2e-559">Tworzenia</span><span class="sxs-lookup"><span data-stu-id="d5a2e-559">Constitution</span></span>                                      |
| <span data-ttu-id="d5a2e-560">starship: długość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-560">starship:length</span></span>       | <span data-ttu-id="d5a2e-561">304.8</span><span class="sxs-lookup"><span data-stu-id="d5a2e-561">304.8</span></span>                                             |
| <span data-ttu-id="d5a2e-562">starship: upoważnione</span><span class="sxs-lookup"><span data-stu-id="d5a2e-562">starship:commissioned</span></span> | <span data-ttu-id="d5a2e-563">False</span><span class="sxs-lookup"><span data-stu-id="d5a2e-563">False</span></span>                                             |
| <span data-ttu-id="d5a2e-564">Znak towarowy</span><span class="sxs-lookup"><span data-stu-id="d5a2e-564">trademark</span></span>             | <span data-ttu-id="d5a2e-565">Paramount obrazy Corp. http://www.paramount.com</span><span class="sxs-lookup"><span data-stu-id="d5a2e-565">Paramount Pictures Corp. http://www.paramount.com</span></span> |

<span data-ttu-id="d5a2e-566">Wywołania aplikacji przykładowej `GetSection` z `starship` klucza.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-566">The sample app calls `GetSection` with the `starship` key.</span></span> <span data-ttu-id="d5a2e-567">`starship` Pary klucz wartość są izolowane.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-567">The `starship` key-value pairs are isolated.</span></span> <span data-ttu-id="d5a2e-568">`Bind` Metoda jest wywoływana w podsekcji, przekazując wystąpienie `Starship` klasy.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-568">The `Bind` method is called on the subsection passing in an instance of the `Starship` class.</span></span> <span data-ttu-id="d5a2e-569">Po powiązaniu wartości wystąpienia, wystąpienie jest przypisany do właściwości w celu renderowania:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-569">After binding the instance values, the instance is assigned to a property for rendering:</span></span>
...
## <a name="bind-to-an-object-graph"></a><span data-ttu-id="d5a2e-570">Powiąż z wykresu obiektu</span><span class="sxs-lookup"><span data-stu-id="d5a2e-570">Bind to an object graph</span></span>

<span data-ttu-id="d5a2e-571"><xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Bind*> jest w stanie całego grafu obiektów POCO powiązania.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-571"><xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Bind*> is capable of binding an entire POCO object graph.</span></span>

<span data-ttu-id="d5a2e-572">Przykładowy zawiera `TvShow` model zawiera wykres obiektu, którego `Metadata` i `Actors` klasy (*Models/TvShow.cs*):</span><span class="sxs-lookup"><span data-stu-id="d5a2e-572">The sample contains a `TvShow` model whose object graph includes `Metadata` and `Actors` classes (*Models/TvShow.cs*):</span></span>
...
<span data-ttu-id="d5a2e-573">Przykładowa aplikacja ma *tvshow.xml* plik zawierający dane konfiguracyjne:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-573">The sample app has a *tvshow.xml* file containing the configuration data:</span></span>
...
<span data-ttu-id="d5a2e-574">Konfiguracja jest powiązany z całej `TvShow` wykres obiektu z `Bind` metody.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-574">Configuration is bound to the entire `TvShow` object graph with the `Bind` method.</span></span> <span data-ttu-id="d5a2e-575">Powiązane wystąpienia jest przypisywany do właściwości w celu renderowania:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-575">The bound instance is assigned to a property for rendering:</span></span>
...
<span data-ttu-id="d5a2e-576">[ConfigurationBinder.Get&lt;T&gt; ](xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Get*) wiąże i zwraca wartość określonego typu.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-576">[ConfigurationBinder.Get&lt;T&gt;](xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Get*) binds and returns the specified type.</span></span> <span data-ttu-id="d5a2e-577">`Get<T>` jest bardziej wygodne niż przy użyciu `Bind`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-577">`Get<T>` is more convenient than using `Bind`.</span></span> <span data-ttu-id="d5a2e-578">Poniższy kod przedstawia sposób użycia `Get<T>` w poprzednim przykładzie, co umożliwia wystąpienie powiązanych, którego można przypisać bezpośrednio do właściwości, używany do renderowania:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-578">The following code shows how to use `Get<T>` with the preceding example, which allows the bound instance to be directly assigned to the property used for rendering:</span></span>
...
## <a name="bind-an-array-to-a-class"></a><span data-ttu-id="d5a2e-579">Powiąż tablicę do klasy</span><span class="sxs-lookup"><span data-stu-id="d5a2e-579">Bind an array to a class</span></span>

<span data-ttu-id="d5a2e-580">*Przykładowa aplikacja pokazuje pojęcia opisane w tej sekcji.*</span><span class="sxs-lookup"><span data-stu-id="d5a2e-580">*The sample app demonstrates the concepts explained in this section.*</span></span>

<span data-ttu-id="d5a2e-581"><xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Bind*> Obsługuje tablice powiązania do obiektów przy użyciu tablicy indeksów w klucze konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-581">The <xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Bind*> supports binding arrays to objects using array indices in configuration keys.</span></span> <span data-ttu-id="d5a2e-582">Dowolnym formacie tablicy, który udostępnia liczbowych segment klucza (`:0:`, `:1:`, &hellip; `:{n}:`) jest w stanie tablicy powiązania z macierzą POCO klasy.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-582">Any array format that exposes a numeric key segment (`:0:`, `:1:`, &hellip; `:{n}:`) is capable of array binding to a POCO class array.</span></span>

> [!NOTE]
> <span data-ttu-id="d5a2e-583">Powiązanie jest zapewniana przez Konwencję.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-583">Binding is provided by convention.</span></span> <span data-ttu-id="d5a2e-584">Konfiguracja niestandardowa dostawców nie są wymagane do zaimplementowania tablicy powiązania.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-584">Custom configuration providers aren't required to implement array binding.</span></span>

<span data-ttu-id="d5a2e-585">**Przetwarzanie w pamięci tablica**</span><span class="sxs-lookup"><span data-stu-id="d5a2e-585">**In-memory array processing**</span></span>

<span data-ttu-id="d5a2e-586">Należy wziąć pod uwagę kluczy i wartości konfiguracji pokazano w poniższej tabeli.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-586">Consider the configuration keys and values shown in the following table.</span></span>

| <span data-ttu-id="d5a2e-587">Key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-587">Key</span></span>     | <span data-ttu-id="d5a2e-588">Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-588">Value</span></span>  |
| :-----: | :----: |
| <span data-ttu-id="d5a2e-589">Macierz: 0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-589">array:0</span></span> | <span data-ttu-id="d5a2e-590">value0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-590">value0</span></span> |
| <span data-ttu-id="d5a2e-591">Macierz: 1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-591">array:1</span></span> | <span data-ttu-id="d5a2e-592">Wartość1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-592">value1</span></span> |
| <span data-ttu-id="d5a2e-593">Macierz: 2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-593">array:2</span></span> | <span data-ttu-id="d5a2e-594">Wartość2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-594">value2</span></span> |
| <span data-ttu-id="d5a2e-595">Macierz: 4</span><span class="sxs-lookup"><span data-stu-id="d5a2e-595">array:4</span></span> | <span data-ttu-id="d5a2e-596">Wartość4</span><span class="sxs-lookup"><span data-stu-id="d5a2e-596">value4</span></span> |
| <span data-ttu-id="d5a2e-597">Macierz: 5</span><span class="sxs-lookup"><span data-stu-id="d5a2e-597">array:5</span></span> | <span data-ttu-id="d5a2e-598">Wartość5</span><span class="sxs-lookup"><span data-stu-id="d5a2e-598">value5</span></span> |

<span data-ttu-id="d5a2e-599">Te klucze i wartości są ładowane w przykładowej aplikacji przy użyciu dostawcy konfiguracji pamięci:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-599">These keys and values are loaded in the sample app using the Memory Configuration Provider:</span></span>
...
<span data-ttu-id="d5a2e-600">Tablica pomija wartość indeksu &num;3.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-600">The array skips a value for index &num;3.</span></span> <span data-ttu-id="d5a2e-601">Konfiguracja obiektu wiążącego nie jest zdolny do powiązania wartości null lub tworzenie wartości null wpisów w powiązanych obiektów, które stają się w chwili, gdy przedstawiono wynik powiązania tej tablicy do obiektu.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-601">The configuration binder isn't capable of binding null values or creating null entries in bound objects, which becomes clear in a moment when the result of binding this array to an object is demonstrated.</span></span>

<span data-ttu-id="d5a2e-602">W przykładowej aplikacji do przechowywania danych konfiguracji powiązanej dostępnej klasy POCO:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-602">In the sample app, a POCO class is available to hold the bound configuration data:</span></span>
...
<span data-ttu-id="d5a2e-603">Dane konfiguracji jest powiązana z obiektem:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-603">The configuration data is bound to the object:</span></span>
...
<span data-ttu-id="d5a2e-604">[ConfigurationBinder.Get&lt;T&gt; ](xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Get*) składni można również użyć, która skutkuje bardziej kompaktowy kod:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-604">[ConfigurationBinder.Get&lt;T&gt;](xref:Microsoft.Extensions.Configuration.ConfigurationBinder.Get*) syntax can also be used, which results in more compact code:</span></span>
...
<span data-ttu-id="d5a2e-605">Powiązany obiekt, wystąpienie `ArrayExample`, odbiera dane tablicy z konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-605">The bound object, an instance of `ArrayExample`, receives the array data from configuration.</span></span>

| <span data-ttu-id="d5a2e-606">`ArrayExamples.Entries` Indeks</span><span class="sxs-lookup"><span data-stu-id="d5a2e-606">`ArrayExamples.Entries` Index</span></span> | <span data-ttu-id="d5a2e-607">`ArrayExamples.Entries` Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-607">`ArrayExamples.Entries` Value</span></span> |
| :---------------------------: | :---------------------------: |
| <span data-ttu-id="d5a2e-608">0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-608">0</span></span>                             | <span data-ttu-id="d5a2e-609">value0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-609">value0</span></span>                        |
| <span data-ttu-id="d5a2e-610">1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-610">1</span></span>                             | <span data-ttu-id="d5a2e-611">Wartość1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-611">value1</span></span>                        |
| <span data-ttu-id="d5a2e-612">2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-612">2</span></span>                             | <span data-ttu-id="d5a2e-613">Wartość2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-613">value2</span></span>                        |
| <span data-ttu-id="d5a2e-614">3</span><span class="sxs-lookup"><span data-stu-id="d5a2e-614">3</span></span>                             | <span data-ttu-id="d5a2e-615">Wartość4</span><span class="sxs-lookup"><span data-stu-id="d5a2e-615">value4</span></span>                        |
| <span data-ttu-id="d5a2e-616">4</span><span class="sxs-lookup"><span data-stu-id="d5a2e-616">4</span></span>                             | <span data-ttu-id="d5a2e-617">Wartość5</span><span class="sxs-lookup"><span data-stu-id="d5a2e-617">value5</span></span>                        |

<span data-ttu-id="d5a2e-618">Indeks &num;3 w powiązany obiekt przechowuje dane konfiguracyjne `array:4` klucz konfiguracji i jego wartość `value4`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-618">Index &num;3 in the bound object holds the configuration data for the `array:4` configuration key and its value of `value4`.</span></span> <span data-ttu-id="d5a2e-619">Gdy dane konfiguracji, które zawiera tablicę jest powiązana, indeksy tablicy w klucze konfiguracji jedynie służą do iteracji dane konfiguracji, podczas tworzenia obiektu.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-619">When configuration data containing an array is bound, the array indices in the configuration keys are merely used to iterate the configuration data when creating the object.</span></span> <span data-ttu-id="d5a2e-620">Wartość null nie mogą być przechowywane w danych konfiguracji, a wpis o wartości null nie jest tworzona w powiązany obiekt, w przypadku tablicy w konfiguracji kluczy Pomiń jeden lub więcej indeksów.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-620">A null value can't be retained in configuration data, and a null-valued entry isn't created in a bound object when an array in configuration keys skip one or more indices.</span></span>

<span data-ttu-id="d5a2e-621">Brak elementu konfiguracji dla indeksu &num;3 mogą być dostarczane przed powiązanie `ArrayExamples` wystąpienie przez dowolnego dostawcę konfiguracji, tworzącego prawidłowe pary klucz wartość w konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-621">The missing configuration item for index &num;3 can be supplied before binding to the `ArrayExamples` instance by any configuration provider that produces the correct key-value pair in configuration.</span></span> <span data-ttu-id="d5a2e-622">Jeśli przykład uwzględniony dodatkowego dostawcę konfiguracji JSON z Brak pary klucz wartość `ArrayExamples.Entries` pasuje do tablicy kompletna konfiguracja:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-622">If the sample included an additional JSON Configuration Provider with the missing key-value pair, the `ArrayExamples.Entries` matches the complete configuration array:</span></span>

<span data-ttu-id="d5a2e-623">*missing_value.JSON*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-623">*missing_value.json*:</span></span>
...
<span data-ttu-id="d5a2e-624">W <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*>:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-624">In <xref:Microsoft.Extensions.Hosting.HostBuilder.ConfigureAppConfiguration*>:</span></span>
...
<span data-ttu-id="d5a2e-625">W `Startup` Konstruktor:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-625">In the `Startup` constructor:</span></span>
...
<span data-ttu-id="d5a2e-626">Pary klucz wartość, jak pokazano w tabeli są ładowane do konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-626">The key-value pair shown in the table is loaded into configuration.</span></span>

| <span data-ttu-id="d5a2e-627">Key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-627">Key</span></span>             | <span data-ttu-id="d5a2e-628">Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-628">Value</span></span>  |
| :-------------: | :----: |
| <span data-ttu-id="d5a2e-629">Macierz: wpisów: 3</span><span class="sxs-lookup"><span data-stu-id="d5a2e-629">array:entries:3</span></span> | <span data-ttu-id="d5a2e-630">Wartość3</span><span class="sxs-lookup"><span data-stu-id="d5a2e-630">value3</span></span> |

<span data-ttu-id="d5a2e-631">Jeśli `ArrayExamples` wystąpienia klasy jest powiązana po dostawcę konfiguracji JSON zawiera wpis dla indeksu &num;3, `ArrayExamples.Entries` tablicy zawiera wartość.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-631">If the `ArrayExamples` class instance is bound after the JSON Configuration Provider includes the entry for index &num;3, the `ArrayExamples.Entries` array includes the value.</span></span>

| <span data-ttu-id="d5a2e-632">`ArrayExamples.Entries` Indeks</span><span class="sxs-lookup"><span data-stu-id="d5a2e-632">`ArrayExamples.Entries` Index</span></span> | <span data-ttu-id="d5a2e-633">`ArrayExamples.Entries` Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-633">`ArrayExamples.Entries` Value</span></span> |
| :---------------------------: | :---------------------------: |
| <span data-ttu-id="d5a2e-634">0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-634">0</span></span>                             | <span data-ttu-id="d5a2e-635">value0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-635">value0</span></span>                        |
| <span data-ttu-id="d5a2e-636">1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-636">1</span></span>                             | <span data-ttu-id="d5a2e-637">Wartość1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-637">value1</span></span>                        |
| <span data-ttu-id="d5a2e-638">2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-638">2</span></span>                             | <span data-ttu-id="d5a2e-639">Wartość2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-639">value2</span></span>                        |
| <span data-ttu-id="d5a2e-640">3</span><span class="sxs-lookup"><span data-stu-id="d5a2e-640">3</span></span>                             | <span data-ttu-id="d5a2e-641">Wartość3</span><span class="sxs-lookup"><span data-stu-id="d5a2e-641">value3</span></span>                        |
| <span data-ttu-id="d5a2e-642">4</span><span class="sxs-lookup"><span data-stu-id="d5a2e-642">4</span></span>                             | <span data-ttu-id="d5a2e-643">Wartość4</span><span class="sxs-lookup"><span data-stu-id="d5a2e-643">value4</span></span>                        |
| <span data-ttu-id="d5a2e-644">5</span><span class="sxs-lookup"><span data-stu-id="d5a2e-644">5</span></span>                             | <span data-ttu-id="d5a2e-645">Wartość5</span><span class="sxs-lookup"><span data-stu-id="d5a2e-645">value5</span></span>                        |

<span data-ttu-id="d5a2e-646">**Przetwarzanie tablicy JSON**</span><span class="sxs-lookup"><span data-stu-id="d5a2e-646">**JSON array processing**</span></span>

<span data-ttu-id="d5a2e-647">Jeśli plik JSON zawiera tablicę, klucze konfiguracji zostaną utworzone dla elementów tablicy przy użyciu indeksu zaczynającego się od zera sekcji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-647">If a JSON file contains an array, configuration keys are created for the array elements with a zero-based section index.</span></span> <span data-ttu-id="d5a2e-648">W następującym pliku konfiguracji `subsection` jest tablicą:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-648">In the following configuration file, `subsection` is an array:</span></span>
...
<span data-ttu-id="d5a2e-649">Dostawca konfiguracji JSON odczytuje dane konfiguracji do następujących par klucz wartość:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-649">The JSON Configuration Provider reads the configuration data into the following key-value pairs:</span></span>

| <span data-ttu-id="d5a2e-650">Key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-650">Key</span></span>                     | <span data-ttu-id="d5a2e-651">Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-651">Value</span></span>  |
| ----------------------- | :----: |
| <span data-ttu-id="d5a2e-652">json_array:key</span><span class="sxs-lookup"><span data-stu-id="d5a2e-652">json_array:key</span></span>          | <span data-ttu-id="d5a2e-653">Wartośća</span><span class="sxs-lookup"><span data-stu-id="d5a2e-653">valueA</span></span> |
| <span data-ttu-id="d5a2e-654">json_array:Subsection:0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-654">json_array:subsection:0</span></span> | <span data-ttu-id="d5a2e-655">Wartośćb</span><span class="sxs-lookup"><span data-stu-id="d5a2e-655">valueB</span></span> |
| <span data-ttu-id="d5a2e-656">json_array:Subsection:1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-656">json_array:subsection:1</span></span> | <span data-ttu-id="d5a2e-657">valueC</span><span class="sxs-lookup"><span data-stu-id="d5a2e-657">valueC</span></span> |
| <span data-ttu-id="d5a2e-658">json_array:Subsection:2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-658">json_array:subsection:2</span></span> | <span data-ttu-id="d5a2e-659">zwracającej</span><span class="sxs-lookup"><span data-stu-id="d5a2e-659">valueD</span></span> |

<span data-ttu-id="d5a2e-660">W przykładowej aplikacji następującej klasy POCO jest dostępny dla powiązania pary klucz wartość konfiguracji:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-660">In the sample app, the following POCO class is available to bind the configuration key-value pairs:</span></span>
...
<span data-ttu-id="d5a2e-661">Po powiązaniu `JsonArrayExample.Key` przechowuje wartość `valueA`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-661">After binding, `JsonArrayExample.Key` holds the value `valueA`.</span></span> <span data-ttu-id="d5a2e-662">Wartości podsekcji są przechowywane we właściwości tablicy POCO `Subsection`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-662">The subsection values are stored in the POCO array property, `Subsection`.</span></span>

| <span data-ttu-id="d5a2e-663">`JsonArrayExample.Subsection` Indeks</span><span class="sxs-lookup"><span data-stu-id="d5a2e-663">`JsonArrayExample.Subsection` Index</span></span> | <span data-ttu-id="d5a2e-664">`JsonArrayExample.Subsection` Wartość</span><span class="sxs-lookup"><span data-stu-id="d5a2e-664">`JsonArrayExample.Subsection` Value</span></span> |
| :---------------------------------: | :---------------------------------: |
| <span data-ttu-id="d5a2e-665">0</span><span class="sxs-lookup"><span data-stu-id="d5a2e-665">0</span></span>                                   | <span data-ttu-id="d5a2e-666">Wartośćb</span><span class="sxs-lookup"><span data-stu-id="d5a2e-666">valueB</span></span>                              |
| <span data-ttu-id="d5a2e-667">1</span><span class="sxs-lookup"><span data-stu-id="d5a2e-667">1</span></span>                                   | <span data-ttu-id="d5a2e-668">valueC</span><span class="sxs-lookup"><span data-stu-id="d5a2e-668">valueC</span></span>                              |
| <span data-ttu-id="d5a2e-669">2</span><span class="sxs-lookup"><span data-stu-id="d5a2e-669">2</span></span>                                   | <span data-ttu-id="d5a2e-670">zwracającej</span><span class="sxs-lookup"><span data-stu-id="d5a2e-670">valueD</span></span>                              |

## <a name="custom-configuration-provider"></a><span data-ttu-id="d5a2e-671">Dostawca konfiguracji niestandardowej</span><span class="sxs-lookup"><span data-stu-id="d5a2e-671">Custom configuration provider</span></span>

<span data-ttu-id="d5a2e-672">Przykładowa aplikacja pokazuje, jak utworzyć dostawcę konfiguracji podstawowej, który odczytuje pary klucz wartość konfiguracji z bazy danych za pomocą [Entity Framework (EF)](/ef/core/).</span><span class="sxs-lookup"><span data-stu-id="d5a2e-672">The sample app demonstrates how to create a basic configuration provider that reads configuration key-value pairs from a database using [Entity Framework (EF)](/ef/core/).</span></span>

<span data-ttu-id="d5a2e-673">Dostawcę ma następującą charakterystykę:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-673">The provider has the following characteristics:</span></span>

* <span data-ttu-id="d5a2e-674">Bazy danych w pamięci programu EF jest używana do celów demonstracyjnych.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-674">The EF in-memory database is used for demonstration purposes.</span></span> <span data-ttu-id="d5a2e-675">Aby użyć bazy danych, która wymaga parametrów połączenia, należy zaimplementować pomocniczy `ConfigurationBuilder` podawać parametrów połączenia z innego dostawcę konfiguracji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-675">To use a database that requires a connection string, implement a secondary `ConfigurationBuilder` to supply the connection string from another configuration provider.</span></span>
* <span data-ttu-id="d5a2e-676">Dostawca odczytuje tabelę bazy danych do konfiguracji podczas uruchamiania.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-676">The provider reads a database table into configuration at startup.</span></span> <span data-ttu-id="d5a2e-677">Dostawca nie kwerendy bazy danych na podstawie-key.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-677">The provider doesn't query the database on a per-key basis.</span></span>
* <span data-ttu-id="d5a2e-678">Załaduj ponownie przy zmianie nie jest zaimplementowany, więc zaktualizowanie bazy danych, po uruchomieniu aplikacji nie ma wpływu na konfigurację aplikacji.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-678">Reload-on-change isn't implemented, so updating the database after the app starts has no effect on the app's configuration.</span></span>

<span data-ttu-id="d5a2e-679">Zdefiniuj `EFConfigurationValue` jednostki do przechowywania wartości konfiguracji w bazie danych.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-679">Define an `EFConfigurationValue` entity for storing configuration values in the database.</span></span>

<span data-ttu-id="d5a2e-680">*Models/EFConfigurationValue.cs*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-680">*Models/EFConfigurationValue.cs*:</span></span>
...
<span data-ttu-id="d5a2e-681">Dodaj `EFConfigurationContext` do przechowywania i uzyskiwania dostępu skonfigurowane wartości.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-681">Add an `EFConfigurationContext` to store and access the configured values.</span></span>

<span data-ttu-id="d5a2e-682">*EFConfigurationProvider/EFConfigurationContext.cs*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-682">*EFConfigurationProvider/EFConfigurationContext.cs*:</span></span>
...
<span data-ttu-id="d5a2e-683">Utwórz klasę, która implementuje <xref:Microsoft.Extensions.Configuration.IConfigurationSource>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-683">Create a class that implements <xref:Microsoft.Extensions.Configuration.IConfigurationSource>.</span></span>

<span data-ttu-id="d5a2e-684">*EFConfigurationProvider/EFConfigurationSource.cs*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-684">*EFConfigurationProvider/EFConfigurationSource.cs*:</span></span>
...
<span data-ttu-id="d5a2e-685">Tworzenie niestandardowego dostawcy konfiguracji przez dziedziczenie z <xref:Microsoft.Extensions.Configuration.ConfigurationProvider>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-685">Create the custom configuration provider by inheriting from <xref:Microsoft.Extensions.Configuration.ConfigurationProvider>.</span></span> <span data-ttu-id="d5a2e-686">Dostawca konfiguracji inicjuje bazy danych, gdy jest on pusty.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-686">The configuration provider initializes the database when it's empty.</span></span>

<span data-ttu-id="d5a2e-687">*EFConfigurationProvider/EFConfigurationProvider.cs*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-687">*EFConfigurationProvider/EFConfigurationProvider.cs*:</span></span>
...
<span data-ttu-id="d5a2e-688">`AddEFConfiguration` Metody rozszerzenia, który pozwala na dodawanie źródła konfiguracji do `ConfigurationBuilder`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-688">An `AddEFConfiguration` extension method permits adding the configuration source to a `ConfigurationBuilder`.</span></span>

<span data-ttu-id="d5a2e-689">*Extensions/EntityFrameworkExtensions.cs*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-689">*Extensions/EntityFrameworkExtensions.cs*:</span></span>
...
<span data-ttu-id="d5a2e-690">Poniższy kod przedstawia sposób używania niestandardowego `EFConfigurationProvider` w *Program.cs*:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-690">The following code shows how to use the custom `EFConfigurationProvider` in *Program.cs*:</span></span>
...
## <a name="access-configuration-during-startup"></a><span data-ttu-id="d5a2e-691">Konfiguracja dostępu podczas uruchamiania</span><span class="sxs-lookup"><span data-stu-id="d5a2e-691">Access configuration during startup</span></span>

<span data-ttu-id="d5a2e-692">Wstrzykiwanie `IConfiguration` do `Startup` konstruktora do dostępu do wartości konfiguracji w `Startup.ConfigureServices`.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-692">Inject `IConfiguration` into the `Startup` constructor to access configuration values in `Startup.ConfigureServices`.</span></span> <span data-ttu-id="d5a2e-693">Uzyskać dostępu do konfiguracji w `Startup.Configure`, albo wstrzyknąć `IConfiguration` bezpośrednio do metody lub użyj wystąpienia z konstruktora:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-693">To access configuration in `Startup.Configure`, either inject `IConfiguration` directly into the method or use the instance from the constructor:</span></span>
...
<span data-ttu-id="d5a2e-694">Na przykład uzyskiwania dostępu do konfiguracji za pomocą metod jako udogodnienie uruchamiania zobacz [uruchamiania aplikacji: wygodne metody](xref:fundamentals/startup#convenience-methods).</span><span class="sxs-lookup"><span data-stu-id="d5a2e-694">For an example of accessing configuration using startup convenience methods, see [App startup: Convenience methods](xref:fundamentals/startup#convenience-methods).</span></span>

## <a name="access-configuration-in-a-razor-pages-page-or-mvc-view"></a><span data-ttu-id="d5a2e-695">Konfiguracja dostępu na stronie stron Razor lub w widoku MVC</span><span class="sxs-lookup"><span data-stu-id="d5a2e-695">Access configuration in a Razor Pages page or MVC view</span></span>

<span data-ttu-id="d5a2e-696">Aby uzyskać dostęp do ustawień konfiguracji na stronie stron Razor lub widoku MVC, należy dodać [użycie dyrektywy](xref:mvc/views/razor#using) ([odwołanie w C#: using — dyrektywa](/dotnet/csharp/language-reference/keywords/using-directive)) dla [Microsoft.Extensions.Configuration przestrzeni nazw ](xref:Microsoft.Extensions.Configuration) i wstawiać <xref:Microsoft.Extensions.Configuration.IConfiguration> do strony lub widoku.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-696">To access configuration settings in a Razor Pages page or an MVC view, add a [using directive](xref:mvc/views/razor#using) ([C# reference: using directive](/dotnet/csharp/language-reference/keywords/using-directive)) for the [Microsoft.Extensions.Configuration namespace](xref:Microsoft.Extensions.Configuration) and inject <xref:Microsoft.Extensions.Configuration.IConfiguration> into the page or view.</span></span>

<span data-ttu-id="d5a2e-697">Na stronie stron Razor:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-697">In a Razor Pages page:</span></span>
...
<span data-ttu-id="d5a2e-698">W widoku MVC:</span><span class="sxs-lookup"><span data-stu-id="d5a2e-698">In an MVC view:</span></span>
...
## <a name="add-configuration-from-an-external-assembly"></a><span data-ttu-id="d5a2e-699">Dodaj konfigurację z zewnętrznego zestawu</span><span class="sxs-lookup"><span data-stu-id="d5a2e-699">Add configuration from an external assembly</span></span>

<span data-ttu-id="d5a2e-700"><xref:Microsoft.AspNetCore.Hosting.IHostingStartup> Implementacji umożliwia dodawanie rozszerzeń do aplikacji przy uruchamianiu z zewnętrznego zestawu poza aplikacji `Startup` klasy.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-700">An <xref:Microsoft.AspNetCore.Hosting.IHostingStartup> implementation allows adding enhancements to an app at startup from an external assembly outside of the app's `Startup` class.</span></span> <span data-ttu-id="d5a2e-701">Aby uzyskać więcej informacji, zobacz <xref:fundamentals/configuration/platform-specific-configuration>.</span><span class="sxs-lookup"><span data-stu-id="d5a2e-701">For more information, see <xref:fundamentals/configuration/platform-specific-configuration>.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="d5a2e-702">Dodatkowe zasoby</span><span class="sxs-lookup"><span data-stu-id="d5a2e-702">Additional resources</span></span>

* <xref:fundamentals/configuration/options>
* [<span data-ttu-id="d5a2e-703">Głębszej analizy konfiguracji firmy Microsoft</span><span class="sxs-lookup"><span data-stu-id="d5a2e-703">Deep Dive into Microsoft Configuration</span></span>](https://www.paraesthesia.com/archive/2018/06/20/microsoft-extensions-configuration-deep-dive/)
