...
Sesja jest dostępny z Razor strony lub widok z `Context.Session`:

[!code-cshtml[](app-state/sample/src/WebAppSessionDotNetCore2.0App/Views/Home/About.cshtml)]

Sesja jest dostępny z `PageModel` klasy lub kontrolera z `HttpContext.Session`. Ta właściwość jest [ISession](/dotnet/api/microsoft.aspnetcore.http.isession) implementacji.
