...
## <a name="setting-up-routing-middleware"></a><span data-ttu-id="d6c6d-114">Konfigurowanie routingu oprogramowania pośredniczącego</span><span class="sxs-lookup"><span data-stu-id="d6c6d-114">Setting up Routing Middleware</span></span>
...
<span data-ttu-id="d6c6d-123">Szablon trasy:</span><span class="sxs-lookup"><span data-stu-id="d6c6d-123">The route template:</span></span>
...
<span data-ttu-id="d6c6d-179">`HttpPostAttribute` ( `[HttpPost]` ) To implementacja `IActionConstraint` umożliwiające tylko akcję, należy wybrać, jeśli polecenie HTTP jest żądaniem `POST`.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-179">The `HttpPostAttribute` ( `[HttpPost]` ) is an implementation of `IActionConstraint` that will only allow the action to be selected when the HTTP verb is `POST`.</span></span> <span data-ttu-id="d6c6d-180">Obecność `IActionConstraint` sprawia, że `Edit(int, Product)` lepiej pasuje niż `Edit(int)`, więc `Edit(int, Product)` zostaną sprawdzone najpierw.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-180">The presence of an `IActionConstraint` makes the `Edit(int, Product)` a 'better' match than `Edit(int)`, so `Edit(int, Product)` will be tried first.</span></span>
...
## <a name="attribute-routing-with-httpverb-attributes"></a><span data-ttu-id="d6c6d-205">Atrybut, routing za pomocą atrybutów Http [polecenie]</span><span class="sxs-lookup"><span data-stu-id="d6c6d-205">Attribute routing with Http[Verb] attributes</span></span>
...
<span data-ttu-id="d6c6d-310">W tym przykładzie użyto trasowanie atrybutów:</span><span class="sxs-lookup"><span data-stu-id="d6c6d-310">This example uses attribute routing:</span></span>
...
> [!NOTE]
> <span data-ttu-id="d6c6d-375">Przestrzeń nazw w każdym kontrolerze przedstawiono tutaj, aby informacje były kompletne — w przeciwnym razie kontrolerów miałby nazewnictwa w konflikcie i generują błędy kompilatora.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-375">The namespace of each controller is shown here for completeness - otherwise the controllers would have a naming conflict and generate a compiler error.</span></span> <span data-ttu-id="d6c6d-376">Klasy w przestrzeni nazw nie mają wpływu na routingu MVC.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-376">Class namespaces have no effect on MVC's routing.</span></span>

<span data-ttu-id="d6c6d-377">Pierwsze dwa kontrolery są elementami członkowskimi obszarów i tylko dopasowania, gdy ich nazwy poszczególnych obszarów są dostarczane przez `area` wartości trasy.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-377">The first two controllers are members of areas, and only match when their respective area name is provided by the `area` route value.</span></span> <span data-ttu-id="d6c6d-378">Trzeci kontroler nie jest elementem członkowskim dowolnego obszaru i może tylko dopasowania, gdy brak wartości parametru `area` są dostarczane przez routingu.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-378">The third controller isn't a member of any area, and can only match when no value for `area` is provided by routing.</span></span>

> [!NOTE]
> <span data-ttu-id="d6c6d-379">Pod względem dopasowania *żadnej wartości*, braku `area` wartość jest taka sama tak, jakby wartość `area` zostały o wartości null ani być ciągiem pustym.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-379">In terms of matching *no value*, the absence of the `area` value is the same as if the value for `area` were null or the empty string.</span></span>

<span data-ttu-id="d6c6d-380">Podczas wykonywania akcji wewnątrz obszaru, trasy wartość `area` będą dostępne jako *otoczenia wartość* routingu służące do generowania adresu URL.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-380">When executing an action inside an area, the route value for `area` will be available as an *ambient value* for routing to use for URL generation.</span></span> <span data-ttu-id="d6c6d-381">Oznacza to, że domyślnie obszarów działania *umocowany* do generowania adresu URL, jak pokazano w następującym przykładzie.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-381">This means that by default areas act *sticky* for URL generation as demonstrated by the following sample.</span></span>
...
## <a name="understanding-iactionconstraint"></a><span data-ttu-id="d6c6d-382">Opis IActionConstraint</span><span class="sxs-lookup"><span data-stu-id="d6c6d-382">Understanding IActionConstraint</span></span>

> [!NOTE]
> <span data-ttu-id="d6c6d-383">Ta sekcja jest rozszerzony na elementy wewnętrzne framework i jak MVC wybiera akcję do wykonania.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-383">This section is a deep-dive on framework internals and how MVC chooses an action to execute.</span></span> <span data-ttu-id="d6c6d-384">Typowa aplikacja nie ma potrzeby niestandardowego `IActionConstraint`</span><span class="sxs-lookup"><span data-stu-id="d6c6d-384">A typical application won't need a custom `IActionConstraint`</span></span>

<span data-ttu-id="d6c6d-385">Prawdopodobnie używano już `IActionConstraint` nawet, jeśli nie znasz przy użyciu interfejsu.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-385">You have likely already used `IActionConstraint` even if you're not familiar with the interface.</span></span> <span data-ttu-id="d6c6d-386">`[HttpGet]` Atrybutu i podobnych zastosowaniach `[Http-VERB]` implementację interfejsu `IActionConstraint` w celu ograniczenia wykonywania metody akcji.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-386">The `[HttpGet]` Attribute and similar `[Http-VERB]` attributes implement `IActionConstraint` in order to limit the execution of an action method.</span></span>
...
<span data-ttu-id="d6c6d-387">Zakładając, że domyślną trasę konwencjonalne ze ścieżką URL `/Products/Edit` dałby w efekcie wartości `{ controller = Products, action = Edit }`, który umożliwi dopasowanie **zarówno** akcji, pokazano poniżej.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-387">Assuming the default conventional route, the URL path `/Products/Edit` would produce the values `{ controller = Products, action = Edit }`, which would match **both** of the actions shown here.</span></span> <span data-ttu-id="d6c6d-388">W `IActionConstraint` terminologii będzie mówi się, że oba te akcje są traktowane jako kandydatów — jak były ze sobą zgodne dane trasy.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-388">In `IActionConstraint` terminology we would say that both of these actions are considered candidates - as they both match the route data.</span></span>

<span data-ttu-id="d6c6d-389">Gdy `HttpGetAttribute` jest wykonywana, jest wyświetlany tekst, który *Edit()* pasuje do *UZYSKAĆ* i nie ma dopasowania dla innych zlecenie HTTP.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-389">When the `HttpGetAttribute` executes, it will say that *Edit()* is a match for *GET* and isn't a match for any other HTTP verb.</span></span> <span data-ttu-id="d6c6d-390">`Edit(...)` Akcji nie ma żadnych ograniczeń zdefiniowanych i dlatego pokaże wszelkie zlecenie HTTP.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-390">The `Edit(...)` action doesn't have any constraints defined, and so will match any HTTP verb.</span></span> <span data-ttu-id="d6c6d-391">Tak zakładając, że `POST` — tylko `Edit(...)` zgodny.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-391">So assuming a `POST` - only `Edit(...)` matches.</span></span> <span data-ttu-id="d6c6d-392">Jednak dla `GET` obie akcje można nadal dopasować — jednak akcję z `IActionConstraint` zawsze jest uważany za *lepsze* niż akcję bez.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-392">But, for a `GET` both actions can still match - however, an action with an `IActionConstraint` is always considered *better* than an action without.</span></span> <span data-ttu-id="d6c6d-393">Dlatego ponieważ `Edit()` ma `[HttpGet]` jest uważany za bardziej szczegółowe i zostanie wybrany, jeżeli może odnosić się do obu akcji.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-393">So because `Edit()` has `[HttpGet]` it's considered more specific, and will be selected if both actions can match.</span></span>

<span data-ttu-id="d6c6d-394">Model `IActionConstraint` jest formą *przeciążenie*, ale zamiast przeciążenia metod o tej samej nazwie, jest przeciążenie między akcjami, które pasują do tego samego adresu URL.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-394">Conceptually, `IActionConstraint` is a form of *overloading*, but instead of overloading methods with the same name, it's overloading between actions that match the same URL.</span></span> <span data-ttu-id="d6c6d-395">Routing atrybutu używa również `IActionConstraint` i może doprowadzić do działania z różnych kontrolerów oba są traktowane jako kandydatów.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-395">Attribute routing also uses `IActionConstraint` and can result in actions from different controllers both being considered candidates.</span></span>

<a name="iactionconstraint-impl-ref-label"></a>

### <a name="implementing-iactionconstraint"></a><span data-ttu-id="d6c6d-396">Implementowanie IActionConstraint</span><span class="sxs-lookup"><span data-stu-id="d6c6d-396">Implementing IActionConstraint</span></span>

<span data-ttu-id="d6c6d-397">Najprostszym sposobem wdrożenia `IActionConstraint` polega na utworzeniu klasy pochodzącej od `System.Attribute` i umieść ją w akcji i kontrolerów.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-397">The simplest way to implement an `IActionConstraint` is to create a class derived from `System.Attribute` and place it on your actions and controllers.</span></span> <span data-ttu-id="d6c6d-398">MVC automatycznie wykryje, dowolny `IActionConstraint` które są stosowane jako atrybuty.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-398">MVC will automatically discover any `IActionConstraint` that are applied as attributes.</span></span> <span data-ttu-id="d6c6d-399">Model aplikacji można użyć, aby zastosować ograniczenia i jest to prawdopodobnie najbardziej elastycznym podejściem, ponieważ umożliwia ona metaprogram, w jaki sposób są one stosowane.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-399">You can use the application model to apply constraints, and this is probably the most flexible approach as it allows you to metaprogram how they're applied.</span></span>

<span data-ttu-id="d6c6d-400">W poniższym przykładzie ograniczenie wybiera akcję na podstawie *numer kierunkowy kraju* z danych trasy.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-400">In the following example a constraint chooses an action based on a *country code* from the route data.</span></span> <span data-ttu-id="d6c6d-401">[Pełny przykład w witrynie GitHub](https://github.com/aspnet/Entropy/blob/master/samples/Mvc.ActionConstraintSample.Web/CountrySpecificAttribute.cs).</span><span class="sxs-lookup"><span data-stu-id="d6c6d-401">The [full sample on GitHub](https://github.com/aspnet/Entropy/blob/master/samples/Mvc.ActionConstraintSample.Web/CountrySpecificAttribute.cs).</span></span>
...
<span data-ttu-id="d6c6d-402">Odpowiedzialność za wdrażanie `Accept` metody i wybierając polecenie "Order" ograniczenia do wykonania.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-402">You are responsible for implementing the `Accept` method and choosing an 'Order' for the constraint to execute.</span></span> <span data-ttu-id="d6c6d-403">W tym przypadku `Accept` metoda zwraca `true` do oznaczania Akcja odnosi się do dopasowania podczas `country` trasy do dopasowania wartości.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-403">In this case, the `Accept` method returns `true` to denote the action is a match when the `country` route value matches.</span></span> <span data-ttu-id="d6c6d-404">To różni się od `RouteValueAttribute` , ponieważ umożliwia powrót do działania z bezatrybutowego.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-404">This is different from a `RouteValueAttribute` in that it allows fallback to a non-attributed action.</span></span> <span data-ttu-id="d6c6d-405">Przykład pokazuje, że jeśli zdefiniujesz `en-US` akcji, a następnie kod kraju, takich jak `fr-FR` powróci do kontrolera bardziej ogólnym, który nie ma `[CountrySpecific(...)]` stosowane.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-405">The sample shows that if you define an `en-US` action then a country code like `fr-FR` will fall back to a more generic controller that doesn't have `[CountrySpecific(...)]` applied.</span></span>

<span data-ttu-id="d6c6d-406">`Order` Właściwość decyduje, które *etapu* ograniczenie jest częścią.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-406">The `Order` property decides which *stage* the constraint is part of.</span></span> <span data-ttu-id="d6c6d-407">Ograniczenia akcji uruchamiania w grupach na podstawie `Order`.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-407">Action constraints run in groups based on the `Order`.</span></span> <span data-ttu-id="d6c6d-408">Na przykład Framework podane atrybuty metody HTTP używać tego samego `Order` wartość, aby były one uruchamiane w tym samym etapie.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-408">For example, all of the framework provided HTTP method attributes use the same `Order` value so that they run in the same stage.</span></span> <span data-ttu-id="d6c6d-409">Może mieć dowolną liczbę etapie, ponieważ musisz zaimplementować żądane zasady.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-409">You can have as many stages as you need to implement your desired policies.</span></span>

> [!TIP]
> <span data-ttu-id="d6c6d-410">Aby wybrać wartość `Order` Pomyśl o czy swoje ograniczenia powinny być stosowane przed metod HTTP.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-410">To decide on a value for `Order` think about whether or not your constraint should be applied before HTTP methods.</span></span> <span data-ttu-id="d6c6d-411">Niższych numerach uruchomiony jako pierwszy.</span><span class="sxs-lookup"><span data-stu-id="d6c6d-411">Lower numbers run first.</span></span>
