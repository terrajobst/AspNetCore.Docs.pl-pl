...
*Web.config* w poniższym przykładzie jest przeznaczony dla [niezależne wdrożenia](https://docs.microsoft.com/dotnet/articles/core/deploying/#self-contained-deployments-scd) do [usłudze Azure App Service](https://azure.microsoft.com/services/app-service/). Aby uzyskać więcej informacji, zobacz [publikowania w usługach IIS](xref:publishing/iis). Zobacz [konfiguracji aplikacji podrzędne](xref:publishing/iis#configuration-of-sub-applications) ważne uwagi dotyczące konfiguracji *web.config* plików w aplikacjach podrzędnych.
