...
4. Uruchom następujące polecenie **Konsola Menedżera pakietów** okno z katalogu głównego projektu:
    ...
    ```
5. Kopiuj *signalr.js* plik z *node_modules\\ @aspnet\signalr\dist\browser*  do *wwwroot\lib* folder w projekcie.
...
1. Utwórz `SendMessage` metodę, która wysyła komunikat do wszystkich klientów połączonych rozmów. Zwróć uwagę, zwraca [zadań](https://msdn.microsoft.com/en-us/library/system.threading.tasks.task(v=vs.110).aspx), ponieważ asynchroniczny SignalR. Kod asynchroniczny skaluje się lepiej.
...
1. Dodaj klasę do projektu, wybierając **pliku** > **nowy plik** z menu.

1. Dziedzicz `Microsoft.AspNetCore.SignalR.Hub`. `Hub` Zawiera klasę, właściwości i zdarzeń związanych z zarządzaniem połączeń i grupy, a także wysyłania i odbierania danych do klientów.

1. Dodaj `SendMessage` metodę do klasy. `SendMessage` Metoda wysyła komunikat do wszystkich klientów połączonych rozmów. Zwróć uwagę, zwraca [zadań](/dotnet/api/system.threading.tasks.task), ponieważ asynchroniczny SignalR. Kod asynchroniczny skaluje się lepiej.
